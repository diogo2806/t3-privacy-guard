...
The structured proposal is persisted and sent to the independent Terminal 3 Rust/WASM policy through authenticated T3N identities. The Tenant, Proposal Agent and Protected Executor have separate sessions and canonical DIDs. A malicious exfiltration proposal receives `DENY`. After that denial, the dashboard does **not** manufacture a safe action. It sends a separate minimum-scope prompt through the same configured AI provider inside the same incident. That second model output is persisted as another independent proposal, its provider/model origin is written to the authenticated local audit chain, and the exact proposal is evaluated by T3N again. `DENY`, `REDACT` or an unsupported action remains visible and blocked; there is no hardcoded fallback. A minimum legitimate model proposal can receive `ALLOW`, but `ALLOW` is still not execution: an authenticated operator must explicitly authorize remediation, after which Spring emits a short-lived one-time signed capability bound to the exact action, decision, **canonical approved destination**, normal fields, logical private references, Protected Executor DID and policy provenance.
...
```text
Minimum-scope credential-remediation prompt
  ...
  -> exact second proposal goes to T3N policy
...
No single success signal is trusted to mean more than it proves. If the second model output is `DENY`, `REDACT` or not `revoke-credential`, the chain stops honestly; the application never substitutes a fixed safe proposal.
...
|---|---|---|---|---|---|
| Credential compromised | `revoke-credential` | `incident-remediation` | `incident_id`, `credential_id`, `reason` | none | Real attack proposal -> T3N DENY -> second real minimum-scope agent proposal in the same incident -> independent T3N decision -> human authorization + exact destination binding + protected execution + independent `REVOKED` read-back when ALLOW |
...
9. Click Ask agent for minimum proposal. This sends a second prompt through the same configured provider and persists a new proposal in the same incident; the application does not create a fixed safe action.
10. Inspect the second provider/model provenance, proposal fields/host and the independent T3N result. If the model returns DENY, REDACT or another action, accept that result as-is; there is no fallback.
11. Continue only on an actual `revoke-credential` ALLOW and confirm the panel shows the exact Approved destination hostname in addition to policy provenance.
...
18. When AI_PROVIDER is live, inspect LIVE-AI-MINIMUM-REMEDIATION. It is PASS only if the configured provider/model produced the minimum proposal and that exact output received T3N ALLOW; disabled provider must remain NOT_RUN.
19. If LIVE-DESTINATION-BINDING was executed, accept it only as PASS when A and B were both policy-allowed yet the contract reported BLOCKED_BEFORE_HTTP for B after A had been approved.
...
- **Denied-to-legitimate recovery is agent-driven.** A new prompt is sent through the same provider in the existing incident. Backend/UI do not author a synthetic safe proposal or force `ALLOW`.
...
- **Spring Boot also owns local business-audit integrity.** Sanitized audit events, including agent provider/model provenance, are chained with a backend-only HMAC key and authenticated head; broken/unverifiable integrity blocks protected local changes without rewriting history.
...
| Configured live model produces the minimum remediation proposal and that exact output receives T3N ALLOW | LIVE OPTIONAL; `NOT_RUN` when provider disabled | `LIVE-AI-MINIMUM-REMEDIATION` in `testnet-run.json` |
...
```text
T3N TEE ALLOW for revoke-credential + host A
...
Agent proposals add a separate sanitized `AGENT_PROPOSAL_SOURCE` event after persistence and before policy evaluation. It binds the action id to the configured provider/model label without persisting the prompt, provider credentials or private values. This makes the two-proposal attack/recovery story independently visible in retained local audit history.
...
`LIVE-AI-MINIMUM-REMEDIATION` is the explicit live-model bridge. When an OpenAI-compatible provider is configured, the runner constructs the same runtime provider/service, sends a minimum remediation prompt containing the approved synthetic host, records only sanitized provider/model and proposal metadata, and evaluates that exact proposal through T3N. It passes only if the provider produced the exact minimum `revoke-credential` shape and T3N returned `ALLOW`. When AI is disabled it is `NOT_RUN`; the deterministic `LIVE-MINIMAL-ALLOW` policy case is kept separate and is never presented as a live-model result.
...
6. **Ask agent for minimum proposal** visible after DENY, followed by the second real provider/model proposal in the same incident; capture that there is no application-generated fallback.
7. Second proposal + independent T3N `ALLOW`/`REDACT`/`DENY`; only continue to protected remediation on an actual `revoke-credential` ALLOW.
...
15. **Evidence** with `Evidence summary`, `Observed outcomes`, then expanded `Technical provenance`; include full Source commit, Source tree state, separate Proposal Agent/Protected Executor DIDs, T3N_TESTNET, contract/WASM/policy provenance, `LIVE-AI-MINIMUM-REMEDIATION` when applicable, and optional scenarios honestly PASS/FAIL/NOT RUN.
...
85–105s  Ask agent for minimum proposal -> same real provider -> second proposal in same incident
105–125s Inspect second proposal + independent T3N ALLOW/REDACT/DENY; show no fallback if blocked
125–140s Show exact Approved destination, then authorize credential revocation only on revoke-credential ALLOW
...
- **agent proposal**: model-produced structured request, not authorization; after a denial, a second proposal must also come from the configured provider rather than application-authored fallback data;
...
- **proved live**: matching live evidence/capture exists. `AI_PROVIDER=disabled` or deterministic policy fixtures cannot support a live-model claim.

Do not say “Verified source” solely because a Git SHA/tree state is present, “Member grant ACTIVE = authorized”, “registered = authorized”, “Agent Card = attestation”, “policy allows B so the human approved B”, “local audit is immutable/tamper-proof”, “local audit VERIFIED = T3N MATCHED”, “guarantees GDPR compliance”, “hardware verified”, “exactly once”, “at most once”, “profile resolution proved live”, “all four scenarios execute end-to-end”, “prompt is PII-free because it passed the guard”, “the safe path is generated by the application” or “completed from HTTP 2xx” without corresponding evidence/contract.
