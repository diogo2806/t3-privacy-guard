...
| A44 | Gateway | Member grant is `ACTIVE` and delegated principal `checkDelegation()` returns `authorised=true` | `memberState=ACTIVE`, `effectiveState=ACTIVE`; exact canonical Tenant DID/functions/scopes are sent by the principal client | `delegation-service.test.ts` | READY_TO_RUN |
| A45 | Gateway | Member grant is `ACTIVE`, but `checkDelegation()` returns false, throws or is inconclusive | `DENIED` or `UNKNOWN`; never effective `ACTIVE` | `delegation-service.test.ts` | READY_TO_RUN |
| A46 | Gateway | Member grant is `SCHEDULED`, `REVOKED`, `NOT_GRANTED` or malformed | No positive effective check is fabricated; effective state remains non-ACTIVE | `delegation-service.test.ts` | READY_TO_RUN |
| A47 | Java/React | Member grant is active but Proposal or Executor effective state is `DENIED`/`UNKNOWN` | Corresponding readiness is false and UI never renders `Operational` | `SystemStatusServiceTest`, `SystemStatusBar.test.tsx`, `TrustFlowSummary.test.tsx` | READY_TO_RUN |
...
| L13 | T3N testnet | Proposal Agent exact effective authorization check | `ACTIVE/ACTIVE` for `evaluate-action` + exact scopes | testnet runner | NOT_RUN until valid credentials/credits |
| L14 | T3N testnet | Protected Executor exact effective authorization check | `ACTIVE/ACTIVE` for `execute-remediation` + `verify-remediation` + exact scopes | testnet runner | NOT_RUN until valid credentials/credits |
| L15 | T3N testnet | Proposal grant is revoked, checked, then minimum grant restored | Revoked check is non-ACTIVE; restored check returns `ACTIVE/ACTIVE` | testnet runner + `EVIDENCE_RUN_EGRESS_NEGATIVES=true` | NOT_RUN until mutable testnet delegation state is enabled |
| L16 | T3N testnet | Executor grant is revoked, checked, then minimum grant restored | Revoked check is non-ACTIVE; restored check returns `ACTIVE/ACTIVE` | testnet runner + `EVIDENCE_RUN_EGRESS_NEGATIVES=true` | NOT_RUN until mutable testnet delegation state is enabled |
...
`Member grant` and `Effective T3N access` are separate evidence dimensions. `ACTIVE` Member state means a readable grant exists inside its validity window. It does not prove the delegated principal is currently authorized. Effective `ACTIVE` requires the authenticated principal itself to call `checkDelegation()` with the canonical Tenant DID and exact contract/functions/scopes and receive `authorised=true`. `authorised=false` is `DENIED`; errors or inconclusive responses are `UNKNOWN`. Both fail closed.

A grant whose `valid_from_secs` is in the future is `SCHEDULED`. Expired grants are `REVOKED`; malformed, non-numeric or inverted validity windows are `UNKNOWN`. Any non-ACTIVE Member state can never be promoted to effective `ACTIVE`.

The Proposal Agent receives only `evaluate-action` and no protected-egress hosts. The Protected Executor uses a different authenticated T3N credential/DID and receives only `execute-remediation` and `verify-remediation` plus the minimum required egress hosts. Their effective checks use separate authenticated clients. The live negative evidence revokes and restores each grant independently and separately attempts both privileged functions with the Proposal Agent session, so either unexpected permission or a false-positive effective check is a failure. A human remediation capability remains required at the application/gateway boundary and is bound to the authenticated Executor DID, so replacing the executor after authorization invalidates execution.
