...
| A40 | Gateway/API/React | Matching Member Delegation has `valid_from_secs` in the future | Member state is `SCHEDULED`; effective access is `INCOMPLETE`; backend readiness remains false; UI renders a pending state and never `Operational` | `delegation-service.test.ts`, `SystemStatusServiceTest`, `SystemStatusBar.test.tsx`, `TrustFlowSummary.test.tsx` | READY_TO_RUN |
| A41 | Gateway | Delegation window contains unreadable bounds or `valid_from_secs > valid_until_secs` | Member state and effective access are `UNKNOWN`, never `ACTIVE` | `delegation-service.test.ts` | READY_TO_RUN |
...
| A44 | Gateway | Member grant is `ACTIVE` and authenticated principal `checkDelegation()` returns `authorised=true` | `effectiveState=ACTIVE`; call uses canonical contract, Tenant session DID as `pii_did`, and exact observed functions/scopes | `delegation-service.test.ts` | READY_TO_RUN |
| A45 | Gateway | Member grant is `ACTIVE` but authenticated principal `checkDelegation()` returns `authorised=false` | `memberState=ACTIVE`, `effectiveState=INCOMPLETE`; platform denial is never promoted to ready | `delegation-service.test.ts` | READY_TO_RUN |
| A46 | Gateway | Member grant is `ACTIVE` but `checkDelegation()` fails or returns no boolean `authorised` | `effectiveState=UNKNOWN`; fail closed; raw SDK objects are not returned | `delegation-service.test.ts` | READY_TO_RUN |
| A47 | Java/API | Member grant is `ACTIVE` while effective state is `INCOMPLETE` or `UNKNOWN` | System status never reports T3N controls ready; readiness depends on effective state for Proposal Agent and Protected Executor | `SystemStatusServiceTest` | READY_TO_RUN |
| A48 | React | Member grant exists but platform verdict is rejected/unavailable | UI separates `Member grant`, `Platform delegation` and `Effective access`; `Operational` / `T3N controls ready` is absent | `SystemStatusBar.test.tsx`, `TrustFlowSummary.test.tsx` | READY_TO_RUN |
...
Delegation status deliberately separates two different facts. `memberState` describes the Member Delegation document observed from the Tenant side: `ACTIVE` means the matching grant is inside its validity window, `SCHEDULED` means its `valid_from_secs` is still in the future, `REVOKED` means it expired, `NOT_GRANTED` means no matching grant exists, and malformed/non-numeric/inverted windows become `UNKNOWN`. `memberState=ACTIVE` is **not** sufficient evidence of operational authority.

When a Member grant is active, the gateway asks T3N again through the authenticated Proposal Agent or Protected Executor client with `checkDelegation(contract, pii_did, functions, scopes)`. The `pii_did` comes only from the authenticated Tenant session and the checked functions/scopes are exactly those observed in the grant. Only `authorised=true` produces `effectiveState=ACTIVE`. `authorised=false` produces `INCOMPLETE`; transport/parsing/unexpected responses produce `UNKNOWN`. Non-active Member states never create an effective success. Proposal and Executor effective states are evaluated independently and both must be `ACTIVE` before the control plane is reported ready.
