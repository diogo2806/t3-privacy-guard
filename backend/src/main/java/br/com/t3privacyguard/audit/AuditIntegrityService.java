...
public class AuditIntegrityService {
    ...
    public AuditEventEntity append(String incidentId, String type, String message) {
        ...
        String eventMac = AuditMac.hmacHex(
            keyring.currentKey(),
            AuditMac.canonicalEvent(keyId, incidentId, eventId, sequence, type, createdAt, message, previousMac)
        );
    ...
    public Verification verify(String incidentId) {
        ...
        if (events.isEmpty()) {
            return broken(events, 0, head.map(AuditChainHeadEntity::getLastEventMac).orElse(null),
                head.isPresent()
                    ? "The authenticated chain head exists but audit events are missing."
                    : "No local audit events are available and no retention purge marker exists.");
        }
        ...
        if (head.isEmpty()) {
            if (events.stream().allMatch(this::isLegacy)) {
                return new Verification(
                    ...
                    null,
                    "legacy",
                    "These events predate local HMAC integrity protection and cannot be retroactively authenticated. Legacy bootstrap is explicit and disabled by default.",
                    List.copyOf(events)
                );
            }
    ...
    private Verification verifyChain(String incidentId, List<AuditEventEntity> events, AuditChainHeadEntity head) {
        ...
        for (int index = secureStart; index < events.size(); index++) {
            ...
            if (!hasCompleteIntegrityMetadata(event)) {
        ...
        for (AuditEventEntity event : protectedEvents) {
            ...
            if (!AuditMac.secureHexEquals(event.getEventMac(), expectedEventMac)) {
                return broken(events, checked, head.getLastEventMac(), "An audit event MAC does not match its persisted content.");
            }
    ...
    private boolean isLegacy(AuditEventEntity event) {
        return !hasAnyIntegrityMetadata(event);
    }
