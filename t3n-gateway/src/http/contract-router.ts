...
export function createContractRouter(
  ...
  router.post('/evaluate', async (request, response) => {
    try { response.json(await service.evaluate(request.body as Omit<PolicyEvaluationRequest, 'agent_did'>)); }
    catch { response.status(503).json({ error: 'Policy evaluation is unavailable' }); }
  });
