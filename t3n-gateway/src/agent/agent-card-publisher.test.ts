...
test('publishes an Agent Card with separate organization owner, admin, and authenticated agent DIDs', async () => {
...
test('adds the authenticated admin as agent-cards writer while preserving existing writers', async () => {
  const calls: Array<{ operation: string; input: unknown }> = [];
  ...
  assert.deepEqual(calls.map((call) => call.operation), ['writers-get', 'writers-set', 'set', 'publish']);
});

test('deduplicates the writer list before replacing the full scope ACL', async () => {
  const calls: Array<{ operation: string; input: unknown }> = [];
  const existingWriter = 'did:t3n:existing-writer';
  const dependencies = createDependencies(() => ({
    writersGet: async () => ({ writers: [existingWriter, existingWriter.toUpperCase(), adminDid] }),
    setWriters: async (input: unknown) => { calls.push({ operation: 'writers-set', input }); },
    agentCardSet: async (input: unknown) => { calls.push({ operation: 'set', input }); },
    agentCardPublish: async (input: unknown) => { calls.push({ operation: 'publish', input }); },
  }));
