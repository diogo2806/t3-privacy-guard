...
test('restores the persisted floor after restart and rejects a lower version', async (t) => {
  const { directory, path } = await fixture();
  t.after(() => rm(directory, { recursive: true, force: true }));

  const beforeRestart = new TrustManifestFloorStore(path, () => new Date('2026-09-12T22:00:00.000Z'));
  await beforeRestart.recordAccepted('testnet', 101);

  const afterRestart = new TrustManifestFloorStore(path, () => new Date('2026-09-12T23:00:00.000Z'));
  assert.equal((await afterRestart.get('testnet'))?.version, 101);
  await assert.rejects(() => afterRestart.recordAccepted('testnet', 100), TrustManifestFloorStoreError);
  assert.equal((await afterRestart.recordAccepted('testnet', 101)).version, 101);
  assert.equal((await afterRestart.recordAccepted('testnet', 102)).version, 102);
});
