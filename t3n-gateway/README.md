...
No EasyPanel, `/data` deve ser uma **Montagem de Volume nomeada e persistente** criada antes do primeiro deploy capaz de registrar ou migrar o contrato, por exemplo `t3-gateway-data -> /data`. A declaração `VOLUME ["/data"]` da imagem Docker não substitui a persistência administrada pelo EasyPanel e pode resultar em volume anônimo que não acompanha a recriação do serviço. O processo continua executando como usuário `node`, não-root, e o volume deve permanecer gravável por esse usuário. Nenhum segredo é incorporado à imagem. Para runtime provisioning, `/data/t3n-runtime-provisioning.json` é apenas um cache local: a reconciliação também mantém uma cópia mínima e durável no mapa privado T3N `privacy-guard-runtime-provisioning`.

Antes de qualquer primeira publicação ou upgrade, valide a persistência com um marcador que sobreviva a uma recriação real do container:

```bash
echo "persistent-$(date -Iseconds)" > /data/persistence-check.txt
cat /data/persistence-check.txt
# faça um novo deploy/recreate do serviço e confirme novamente:
cat /data/persistence-check.txt
```

Se o segundo `cat` não devolver o mesmo conteúdo, não prossiga com registro/migração T3N. Corrija a montagem persistente primeiro.
...
configurar private remediation maps somente com SECURITY_* reais e válidos
      |
      +--> mapa existente + numericContractId: readers.only=[numericContractId] e writers.only=[] são reconciliados antes da escrita administrativa
      +--> execução manual sem numericContractId: somente mapa já existente e ativo pode ser reutilizado após `tenant.maps.getStatus(...)`
...
Operações de control plane da T3N podem exigir créditos na conta autenticada. Antes de `contract:setup-policy`, `contract:setup-remediation` ou outra mutação administrativa, a conta deve possuir crédito suficiente. `InsufficientCredit` é um estado externo da T3N/testnet: a ação operacional é reabastecer os créditos e repetir o provisioning, não alterar ACLs, secrets ou o código para contornar a cobrança. Os helpers preservam somente a categoria sanitizada `INSUFFICIENT_CREDIT`; valores como account, `required`, `available`, payloads privados e credenciais nunca são propagados. Divergência de read-back permanece um diagnóstico separado de falha RPC/crédito.
...
Falha em qualquer etapa é sanitizada, registrada sem secrets e mantém o sistema fail-closed. O servidor HTTP continua observável para que `/health` e os endpoints de status indiquem o que ainda não está pronto; uma falha de provisionamento nunca é convertida em readiness positivo. Se uma falha ocorrer depois de um registro T3N bem-sucedido, o cache local do numeric id já permanece gravado para permitir que o próximo startup reutilize o contrato sem tentar registrar novamente a mesma versão; isso não torna policy, remediation ou delegações prontas por si só.

O bootstrap de `privacy-guard-policy` trata `current` e `version:<version>` como entradas opcionais somente antes do primeiro publish. Se uma dessas leituras ainda não existir, a publicação pode prosseguir; falha de escrita nunca é ignorada. A reconciliação só considera a policy ativa depois de gravar `current` e fazer read-back estrito da mesma versão e hash pela superfície `tenant.maps` do Tenant autenticado. Erro ou divergência no read-back continua falhando fechado.

`GET /internal/contracts/privacy-guard/enterprise-integration-readiness` expõe `diagnosticCode` apenas por allowlist quando o estado é `UNKNOWN`; o backend propaga o mesmo código em `/api/system/status`. Os códigos possíveis são `POLICY_UNAVAILABLE`, `POLICY_INVALID`, `PRIVATE_CONFIGURATION_UNAVAILABLE`, `ENDPOINT_CONFIGURATION_INVALID`, `DELEGATION_UNAVAILABLE`, `INSUFFICIENT_CREDIT` e `T3N_CONTROL_PLANE_UNAVAILABLE`; `NONE` indica ausência de diagnóstico de falha. `INSUFFICIENT_CREDIT` informa somente que a conta T3N autenticada precisa ser reabastecida antes de repetir o provisioning; o status não expõe account, saldo, crédito exigido ou qualquer segredo. Nenhum código contém mensagem bruta do SDK, segredo, URL privada completa, path, query, fragment ou stack trace. O readiness lê policy e configuração protegida pelo acesso administrativo autenticado do Tenant, sem adicionar o Tenant a `readers.only`; `UNKNOWN` permanece fail-closed e nunca é promovido a `READY` somente por variáveis de ambiente.
...
O comando usa o WASM empacotado na imagem e o adapter compartilhado `registerContractWithDurableId`, que chama `tee:tenant/contracts::contract-register` via `T3nClient.executeWithBlob` e retorna `numericContractId`. No fluxo manual, copie somente esse identificador numérico para a configuração de runtime:

```text
T3N_CONTRACT_NUMERIC_ID=<numericContractId retornado pela T3N>
```

Nunca invente ou antecipe esse valor. Reinicie/reimplante o serviço com o valor real antes das etapas seguintes quando algum mapa privado precisar ser criado ou ter o ACL reparado. Quando a reconciliação automática registra ou atualiza o contrato, esse valor é persistido no cache `/data` antes de qualquer verificação pós-registro e no mapa privado de runtime state depois da confirmação da identidade, sem precisar ser copiado manualmente.

### 2. Publicar a policy operacional

`T3N_CONTRACT_NUMERIC_ID` é obrigatório quando o mapa privado `privacy-guard-policy` precisa ser criado ou quando o ACL precisa ser repontado ao contrato atual. Com um numeric id real, mapa existente é atualizado para `readers.only=[numericContractId]` e `writers.only=[]` antes da publicação. Isso mantém a leitura de negócio disponível somente para o contrato e reserva a mutação ao caminho administrativo autenticado do Tenant. Se o mapa já está corretamente acessível e nenhum numeric id local existe, o script ainda pode publicar/read-back a policy usando `TenantClient.maps.entrySet/entryGet` como proprietário autenticado, sem inventar o id nem alterar o ACL.
...
A policy padrão vem de `/app/policy/privacy-guard-policy.json`. O script grava a versão/hash na KV privada e verifica por read-back cada escrita administrativa; a ativação de `current` continua exigindo versão/hash compatíveis. `T3N_POLICY_FILE` pode ser sobrescrita explicitamente apenas quando houver um arquivo de policy válido montado no container. Se o mapa estiver ausente e nenhum numeric id real estiver disponível, a operação falha fechada em vez de criar uma ACL ampla. Se a T3N responder `InsufficientCredit`, o comando preserva somente o diagnóstico sanitizado e orienta reabastecer os créditos antes de repetir a publicação.
...
`T3N_CONTRACT_NUMERIC_ID` também é necessário para criar `secrets`/`privacy-guard-execution-nonces` ou reparar seus ACLs. Com id real, cada mapa existente é reconciliado para `readers.only=[numericContractId]` e `writers.only=[]` por `tenant.maps.update(...)`; cada valor é semeado por `TenantClient.maps.entrySet(...)` e confirmado por `TenantClient.maps.entryGet(...)` antes de o setup concluir. Esses ACLs continuam limitando o acesso dos contratos e não concedem leitura pública/ampla; o Tenant/Admin autenticado faz o read-back pela superfície administrativa do mapa. Se os mapas já existem e estão ativos, as entradas continuam reconciliáveis sem um numeric id local; `tenant.maps.getStatus(...)` confirma a existência sem depender de um probe de leitura sujeito ao ACL do contrato. Quando um desses mapas está ausente e o numeric id não está disponível, o script retorna um erro sanitizado informando que `T3N_CONTRACT_NUMERIC_ID` é necessário para criar/reparar o mapa com segurança, em vez de propagar o erro RPC bruto.
...
Como `setWriters` substitui o documento de writers do scope, uma falha em `writersGet` ou `setWriters` interrompe a publicação antes de `agentCardSet`; o código nunca trata uma leitura falha como lista vazia nem remove outros writers para conceder esse acesso ao admin atual.
...
resolve current contract
      ...
      |
      v
create/re-ACL private maps with readers.only=[numeric id] and writers.only=[]
      |
      +--> Tenant/Admin owner access: maps.entrySet -> maps.entryGet exact read-back
      |
      v
...
- o state remoto contém apenas Tenant DID, contract id/version, numeric id e timestamp, é privado, usa `readers.only=[numericContractId]` e `writers.only=[]`, e exige `TenantClient.maps.entrySet(...)` seguido de `entryGet(...)` compatível após gravação administrativa; ele só é gravado depois de a identidade publicada ser confirmada;
- uma versão T3N superior ao artefato empacotado falha fechada; o runtime nunca faz downgrade automático;
- ao receber um numeric id real, mapas privados administrativos existentes são reparados para `readers.only=[numericContractId]` e `writers.only=[]` antes da publicação de policy/secrets, sem conceder escrita de negócio ao contrato;
- `contract:setup-policy` e `contract:setup-remediation` nunca inventam numeric id nem usam ACL ampla; sem id local, somente mapas já existentes/ativos podem ser reconciliados pela superfície administrativa autenticada do Tenant;
- `contract:setup-policy` e `contract:setup-remediation` exigem read-back das gravações administrativas por `tenant.maps.entryGet(...)` e falham fechados em qualquer divergência;
- falhas `InsufficientCredit` em operações administrativas são classificadas como `INSUFFICIENT_CREDIT`, sem account, `required`, `available`, segredo ou payload privado; o operador deve reabastecer os créditos da T3N e repetir o provisioning, enquanto read-back mismatch, autenticação, acesso e rede permanecem diagnósticos distintos;
