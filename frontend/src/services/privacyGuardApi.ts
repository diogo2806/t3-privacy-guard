...
export type AuditIntegrityState = 'VERIFIED' | 'BROKEN' | 'LEGACY_UNVERIFIED' | 'PURGED' | 'NOT_AVAILABLE';
...
export interface AuditIntegrity { state: AuditIntegrityState; eventsChecked: number; head?: string | null; version?: string | null; message: string; }
...
export interface AuditEvidence { localEvents: LocalAuditEvidence[]; t3nEvents: T3nActivityEvidence[]; integrity: AuditIntegrity; provenance: AuditProvenance; nextSequence?: number | null; limit: number; }
