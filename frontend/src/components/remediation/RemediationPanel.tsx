...
export function RemediationPanel({ action, decision, execution, busy, onAuthorize, onExecute, onVerify }: Props) {
  ...
  return (
    <section className="card remediation-card" aria-labelledby="remediation-title">
      ...
      {canExecute && <>
        <div className="success-state"><ShieldCheck aria-hidden="true" /><span>Human authorization binds the exact destination, trusted normal payload and private-reference set. T3N will serialize only the policy-allowed normal subset and resolve private data inside protected execution.</span></div>
