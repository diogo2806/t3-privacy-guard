...
describe('SystemStatusBar', () => {
  ...
  it('renders a scheduled proposal delegation as pending and never operational', () => {
    render(<SystemStatusBar status={{ ...status('REGISTERED'), evaluationReady: false, protectedRemediationReady: false, delegationState: 'SCHEDULED', delegationEffectiveState: 'DENIED', message: 'Proposal delegation exists, but its authorization window has not begun.' }} loading={false} onRefresh={vi.fn()} />);
  ...
  it('renders a scheduled executor delegation as pending and never operational', () => {
    render(<SystemStatusBar status={{ ...status('REGISTERED'), protectedRemediationReady: false, executorDelegationState: 'SCHEDULED', executorDelegationEffectiveState: 'DENIED', message: 'Executor delegation exists, but its authorization window has not begun.' }} loading={false} onRefresh={vi.fn()} />);
    expect(screen.getAllByText('Scheduled').length).toBeGreaterThan(0);
    expect(screen.queryByText('Operational')).not.toBeInTheDocument();
    expect(screen.getByText(/authorization window has not begun/i)).toBeInTheDocument();
  });
