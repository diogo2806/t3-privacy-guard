...
describe('ScreenManualDialog', () => {
  it('traps focus and distinguishes Member grant from effective T3N authorization', async () => {
    ...
    expect(dialog).toHaveTextContent(/Member grant.*não é apresentado como prova suficiente/i);
    expect(dialog).toHaveTextContent(/checkDelegation\(\).*cliente autenticado do próprio principal/i);
    expect(dialog).toHaveTextContent(/Confirmed.*authorised=true/i);
    expect(dialog).toHaveTextContent(/Denied e Unknown sempre falham fechado/i);
    expect(dialog).toHaveTextContent(/Proposal Agent e Protected Executor têm checks independentes/i);
