      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p>O A2A expõe somente avaliação pública compatível com Proposal + evaluate-action. Não expõe autorização humana, prova one-time, valores normais, referências privadas resolvidas, execute-remediation, verify-remediation ou credencial do Executor. A2A configurado/publicado não prova reachability.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p>Evidence summary mostra PASS/FAIL/NOT RUN, rede, <strong>Source tree</strong>, trust anchor, policy e horário. PASS exige observação compatível; NOT RUN não conta como prova. A evidence não publica username, e-mail, recipient plaintext, hash do operador, prova assinada, nonce nem chave privada. O fingerprint da chave pública pode ser metadata não secreta. <strong>LIVE-PROFILE-PLACEHOLDER-RESOLUTION</strong> permanece NOT RUN por padrão e só pode virar PASS após execução real opt-in em T3N testnet com perfil sintético contendo e-mail verificado, side effect controlado e read-back DELIVERED com <code>recipient_resolved=true</code>. <strong>COMPLETED</strong> continua dependente de verificação independente.</p>
          </section>
          <section>
            <h3>Fluxo principal</h3>
            <p>1. Leia readiness e Enterprise integration. 2. Use Executive demo apenas para apresentação. 3. No Protection flow, escolha cenário e Analyze with agent. 4. Revise proposal, policy e minimização. 5. Em ALLOW/REDACT executável, confira os campos mínimos e, em notify-security, somente a referência <code>verified_email</code>. 6. Registre Human authorization. 7. Confirme Protected Executor. 8. Execute e observe One-time authorization proof, Gateway proof check e T3N execution proof check sem revelar o token ou recipient. 9. Verifique o estado externo específico da ação. 10. Somente após REVOKED ou DELIVERED + recipient_resolved aceite COMPLETED. 11. Em Evidence, confira summary, outcomes e provenance.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Falha de provider, T3N, Agent Card, A2A, policy KV, delegation ou trust boundary nunca é apresentada como sucesso. Prova humana ausente, expirada, assinatura inválida, key id inválido, body/Executor incompatível ou nonce já consumido falha fechado antes do protected egress. Para notify-security, private ref ausente, extra, literal de placeholder, não permitido pela policy, DELIVERED sem <code>recipient_resolved=true</code> ou erro de resolução permanecem bloqueados/UNVERIFIED sem ecoar recipient. REAUTHORIZATION REQUIRED exige nova aprovação explícita. Destination changed exige nova ação, avaliação e autorização. Aceitação sem read-back permanece PENDING_VERIFICATION ou UNVERIFIED. Audit integrity broken bloqueia mudanças protegidas. Evidence ausente permanece Live evidence not loaded/generated.</p>
          </section>
        </div>
      </section>
    </div>,
