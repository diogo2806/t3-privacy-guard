      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p>O fluxo completo atual é <code>revoke-credential</code>. <strong>Authorize credential revocation</strong> registra a decisão humana vinculada à decisão persistida e à policy version/hash; antes da mudança, a cadeia do audit local deve continuar verificável. A capability de uso único inclui também a Protected Executor DID autenticada no momento da autorização. <strong>Execute protected credential revocation</strong> exige novamente uma cadeia local verificável, envia essa capability ao gateway, rejeita troca de executor e usa a sessão T3N do Protected Executor. O Proposal Agent não possui grant T3N para essa função. A policy é revalidada antes do egress. Aceitação externa gera estado pendente; <strong>Verify external state</strong> também falha fechado se a integridade local estiver quebrada e faz read-back independente. <strong>COMPLETED</strong> só aparece após confirmação do estado esperado. Resultado ambíguo permanece <strong>UNVERIFIED</strong> e não é reenviado automaticamente.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p>O audit local registra eventos de negócio sanitizados em uma cadeia HMAC-SHA256 por incidente. Cada evento autenticado inclui sequência monotônica, vínculo com o MAC anterior e metadados T3N persistidos; um chain-head autenticado permite detectar também remoção da cauda. A chave HMAC é uma credencial de runtime separada e não aparece na API ou interface. Este mecanismo é <strong>tamper-evident</strong>: ele detecta alterações verificáveis no banco, mas não torna o H2 imutável e não substitui armazenamento externo independente.</p>
            <p><strong>VERIFIED</strong> significa que conteúdo, ordem, links, MACs e chain-head do histórico retido foram verificados. <strong>BROKEN</strong> significa divergência ou lacuna detectada e bloqueia mudanças protegidas. <strong>KEY_MISMATCH</strong> indica que uma versão de chave necessária não está disponível ou não confirma a cadeia. <strong>LEGACY_UNVERIFIED</strong> identifica eventos anteriores à proteção HMAC; eles não são retroativamente apresentados como autenticados. Migração de legado para uma nova raiz de confiança exige ação explícita e fica desativada no fluxo normal.</p>
            <p>O <strong>T3N Activity Log</strong> é uma segunda fonte read-only de provenance de rede. <strong>MATCHED</strong> exige correspondência exata de sequence/hash e identidade técnica esperada. <strong>UNMATCHED</strong> significa que havia provenance esperada sem confirmação exata. <strong>LOCAL ONLY</strong> é normal para eventos apenas da aplicação. <strong>T3N ONLY</strong> indica evento T3N relevante sem vínculo local. Uma janela truncada nunca é apresentada como prova de ausência. Eventos de avaliação devem identificar o Proposal Agent; eventos de execução/verificação devem identificar o Protected Executor. Integridade local VERIFIED não fabrica MATCHED, e indisponibilidade do Activity Log não transforma a cadeia local em BROKEN.</p>
          </section>
          <section>
            <h3>Proof &amp; evidence</h3>
            <p><strong>PASS</strong> significa resultado observado compatível com o esperado. <strong>FAIL</strong> é divergência observada. <strong>NOT RUN</strong> não conta como prova. O bundle identifica o SHA completo do <strong>Source commit</strong> e o estado <strong>Source tree</strong> como CLEAN ou DIRTY, além de rede, SDK, tenant DID, Proposal Agent DID, Protected Executor DID, Agent Card URI/hash/status, contrato, WASM SHA-256, policy version/hash e trust-manifest provenance. O source commit liga a execução à revisão pública do código; ele não substitui WASM/policy hashes nem significa auditoria independente. CLEAN informa apenas que a árvore Git estava sem alterações quando a geração começou; DIRTY permanece explícito e não é promovido a sucesso. O cenário <strong>LIVE-PROPOSAL-CANNOT-EXECUTE</strong>, quando executado, deve provar que a própria Member Delegation T3N rejeita tentativa do Proposal Agent de chamar a função privilegiada. Agent Card prova onboarding público, não autorização nem attestation de hardware.</p>
          </section>
          <section>
            <h3>Permissões e regras</h3>
            <p>Login na aplicação não concede autoridade T3N. As DIDs do Tenant, Proposal Agent e Protected Executor vêm de sessões autenticadas, nunca de valores hardcoded. O Agent Card não concede funções. Member grant observado não equivale sozinho a effective access. O Proposal Agent só é tratado como autorizado quando sua própria sessão passa em <code>checkDelegation()</code>; o mesmo vale para o Protected Executor. A Proposal delegation não pode conter funções de remediação; a Executor delegation não deve ser usada pelo fluxo de proposta. Rotas internas usam service token. A chave de integridade do audit é independente do service token, da capability de remediação e da senha do operador. A interface não promove DENY/REDACT para execução, não inventa receipt T3N, não chama audit HMAC de imutável e não transforma indisponibilidade em sucesso.</p>
          </section>
          <section>
            <h3>Fluxo principal</h3>
            <p>1. Entre na aplicação. 2. Confira status de Tenant, Proposal Agent e Protected Executor. 3. Diferencie Agent Card, Member grant, Platform delegation e Effective access. 4. Confirme que os dois effective access estão ACTIVE antes de interpretar o control plane como pronto. 5. Escolha um cenário e revise o prompt. 6. Use Ask agent. 7. Inspecione a proposta real. 8. Leia a decisão T3N e sua policy version/hash. 9. Confira a integridade do audit local separadamente da provenance T3N. 10. Se houver ALLOW para revogação suportada e a cadeia local estiver verificável, registre autorização humana. 11. Confirme que a autorização está vinculada ao Protected Executor. 12. Execute uma vez. 13. Verifique o estado externo. 14. Consulte Execution Trace, integridade local, T3N provenance e Proof &amp; evidence; no bundle live, confira Source commit, Source tree, WASM SHA-256 e Policy SHA-256 como identidades distintas.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Quando uma classe suportada é detectada, o prompt é rejeitado antes do provider remoto e a resposta não ecoa o valor ou offset detectado. Remova o literal e use contexto não sensível ou uma referência lógica aprovada quando existir. Falha de provedor, T3N, Agent Card, policy KV, Proposal Agent, Protected Executor, <code>checkDelegation()</code> ou trust boundary não é apresentada como sucesso. Card ausente gera <strong>NOT REGISTERED</strong>; divergência de DID/schema gera <strong>CARD/DID MISMATCH</strong>; indisponibilidade de resolução gera <strong>UNAVAILABLE</strong>. Member grant <strong>SCHEDULED</strong>, <strong>REVOKED</strong> ou <strong>NOT GRANTED</strong> permanece não operacional. <code>authorised=false</code> resulta em effective access <strong>INCOMPLETE</strong>; erro ou resposta inválida resulta em <strong>UNKNOWN</strong>. Troca da Executor DID entre autorização e execução invalida a capability. Falha no Activity Log mantém o audit local disponível e marca a provenance de rede como não verificada. <strong>BROKEN</strong> ou <strong>KEY_MISMATCH</strong> no audit local bloqueia mudanças protegidas e nunca é corrigido automaticamente; <strong>LEGACY_UNVERIFIED</strong> permanece explicitamente não verificado até uma migração autorizada. Mudança de policy após autorização bloqueia execução protegida. Sessão expirada exige novo login.</p>
          </section>
        </div>
      </section>
    </div>,
