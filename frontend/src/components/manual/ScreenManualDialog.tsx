      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p><strong>Member grant</strong> informa qual autorização foi configurada e lida para o principal, incluindo janela, funções, scopes e hosts. Ele não é apresentado como prova suficiente de que a chamada é autorizada naquele instante. <strong>Effective T3N access</strong> é o resultado de <code>checkDelegation()</code> executado pelo cliente autenticado do próprio principal, usando o contrato, a Tenant DID canônica e apenas as funções/scopes mínimos do fluxo. <strong>Confirmed</strong> aparece somente quando a T3N retorna <code>authorised=true</code>; <strong>Denied</strong> corresponde a <code>authorised=false</code>; <strong>Unknown</strong> cobre erro, timeout ou resposta inconclusiva. Denied e Unknown sempre falham fechado.</p>
            <p>Proposal Agent e Protected Executor têm checks independentes. O Proposal é confirmado apenas para <code>evaluate-action</code>; o Executor é confirmado apenas para <code>execute-remediation</code> e <code>verify-remediation</code>. Um check não pode ser reutilizado para provar a autoridade do outro principal. Grants agendados, revogados ou ausentes nunca são promovidos a acesso efetivo confirmado.</p>
            <p><strong>Registered</strong> significa que um Agent Card público válido foi resolvido para exatamente a mesma Proposal Agent DID. Registrar o Agent Card aumenta descoberta pública, mas não concede acesso ao contrato. O fluxo não é considerado operacional se a sessão necessária estiver indisponível ou se o acesso efetivo correspondente não estiver confirmado.</p>
          ...
          <section>
            ...
            <p><strong>PASS</strong> significa resultado observado compatível com o esperado. <strong>FAIL</strong> é divergência observada. <strong>NOT RUN</strong> não conta como prova. O bundle identifica rede, SDK, tenant DID, Proposal Agent DID, Protected Executor DID, Agent Card URI/hash/status, contrato, WASM SHA-256, policy version/hash e trust-manifest provenance. O live evidence também registra estados sanitizados de Member grant e Effective T3N access, com as funções/scopes exatas consultadas, sem incluir o documento completo de autorização. O cenário <strong>LIVE-PROPOSAL-CANNOT-EXECUTE</strong>, quando executado, deve provar que a própria Member Delegation T3N rejeita tentativa do Proposal Agent de chamar a função privilegiada. Agent Card prova onboarding público, não autorização nem attestation de hardware.</p>
          </section>
          <section>
            <h3>Permissões e regras</h3>
            <p>Login na aplicação não concede autoridade T3N. As DIDs do Tenant, Proposal Agent e Protected Executor vêm de sessões autenticadas, nunca de valores hardcoded. O Agent Card não concede funções. A existência de um Member grant não basta para marcar o fluxo como pronto: o principal correspondente deve obter <strong>Confirmed</strong> no check efetivo. Proposal e Executor usam funções/scopes mínimos e checks independentes, sem wildcard. Rotas internas usam service token. A interface não promove DENY/REDACT para execução, não inventa receipt T3N e não transforma indisponibilidade em sucesso.</p>
          </section>
          <section>
            <h3>Fluxo principal</h3>
            <p>1. Entre na aplicação. 2. Confira a autenticação de Tenant, Proposal Agent e Protected Executor. 3. Diferencie Agent Card, Member grant e Effective T3N access. 4. Confirme que o Proposal mostra acesso efetivo para avaliação e que o Executor mostra acesso efetivo para execução/verificação antes de tratar o fluxo como pronto. 5. Escolha um cenário e revise o prompt. 6. Use Ask agent. 7. Inspecione a proposta real. 8. Leia a decisão T3N e sua policy version/hash. 9. Se houver ALLOW para revogação suportada, registre autorização humana. 10. Confirme que a autorização está vinculada ao Protected Executor. 11. Execute uma vez. 12. Verifique o estado externo. 13. Consulte Execution Trace, audit provenance e Proof &amp; evidence.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Conteúdo sensível é rejeitado sem ecoar o literal. Falha de provedor, T3N, Agent Card, policy KV, Proposal Agent, Protected Executor ou trust boundary não é apresentada como sucesso. Member grant ausente, revogado ou agendado mantém o fluxo fechado; um grant Active com <code>authorised=false</code> aparece como <strong>Denied</strong>; erro, timeout ou resposta inconclusiva do check aparece como <strong>Unknown</strong>. Nenhum desses estados é apresentado como Ready ou Operational. Card ausente gera <strong>NOT REGISTERED</strong>; divergência de DID/schema gera <strong>CARD/DID MISMATCH</strong>; indisponibilidade de resolução gera <strong>UNAVAILABLE</strong>. Troca da Executor DID entre autorização e execução invalida a capability. Falha no Activity Log mantém o audit local disponível e marca a provenance de rede como não verificada. Mudança de policy após autorização bloqueia execução protegida. Sessão expirada exige novo login.</p>
          </section>
        </div>
      </section>
    </div>,
