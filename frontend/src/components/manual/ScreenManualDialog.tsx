      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p>1. Entre na aplicação. 2. Leia o readiness e o <strong>Current result</strong> no Trust Flow. 3. Abra <strong>System readiness details</strong> quando precisar diagnosticar Tenant, Proposal Agent, Protected Executor, grants ou acesso efetivo. 4. Em <strong>Protection flow</strong>, escolha o cenário e revise o prompt. 5. Use <strong>Analyze with agent</strong>. 6. Inspecione a primeira proposta, a decisão e o <strong>Approved destination</strong>. 7. Se houver DENY no cenário de credencial comprometida, use <strong>Ask agent for minimum proposal</strong>; isso chama novamente o provider real no mesmo incidente, gera uma segunda proposta independente e a submete à mesma policy T3N. 8. Inspecione a segunda proposta no histórico de ações e confirme o resultado real da policy; DENY/REDACT ou uma ação sem executor protegido permanecem bloqueados. 9. Se a policy estiver PENDING, use o retry dentro da etapa Policy. 10. Em ALLOW para <code>revoke-credential</code>, registre autorização humana quando exigida. 11. Confirme novamente o Approved destination e execute pelo Protected Executor. 12. Verifique o estado externo. 13. Em <strong>Evidence</strong>, leia summary e outcomes antes de abrir provenance técnica. A2A pode reproduzir somente a etapa pública de proposta/avaliação.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Falha de provedor, T3N, Agent Card, A2A, policy KV, Proposal Agent, Protected Executor, <code>checkDelegation()</code> ou trust boundary nunca é apresentada como sucesso. Se o provider falhar ao pedir a proposta de remediação, nenhuma ActionProposal sintética é criada e nenhum ALLOW é fabricado. <code>authorised=false</code> resulta em Denied; erro ou resposta inválida resulta em Unknown. <strong>Audit integrity broken</strong> bloqueia mudanças protegidas; chave histórica ausente é exibida como KEY_MISMATCH e não como VERIFIED. Troca da Executor DID invalida a capability. Mudança de policy após autorização bloqueia execução. <strong>Destination changed</strong> significa que o hostname configurado no KV privado deixou de corresponder ao Approved destination vinculado à autorização humana; o contrato bloqueia antes do egress e a interface orienta criar/reavaliar e autorizar novamente a ação. Host ausente ou enviado como URL, porta, path ou credencial também falha fechado. Prompt rejeitado informa somente a categoria detectada, sem ecoar valor ou offset. Sessão expirada exige novo login.</p>
          </section>
        </div>
      </section>
    </div>,
