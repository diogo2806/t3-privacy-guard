      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p>Falha de provedor, T3N, Agent Card, A2A, policy KV, Proposal Agent, Protected Executor, <code>checkDelegation()</code> ou trust boundary nunca é apresentada como sucesso. <code>authorised=false</code> resulta em Denied; erro ou resposta inválida resulta em Unknown. <strong>REDACT</strong> só é executável quando o mínimo obrigatório sobrevive e o subconjunto reavaliado vira ALLOW. Payload confiável ausente, chave não solicitada, campo secreto ou valor alterado após autorização falha fechado. <strong>Destination changed</strong> exige nova ação, avaliação e autorização. Aceitação sem read-back permanece pendente ou UNVERIFIED. <strong>Audit integrity broken</strong> bloqueia mudanças protegidas. Prompt rejeitado informa somente a categoria detectada, sem ecoar valor ou offset. Sessão expirada exige novo login.</p>
          </section>
        </div>
      </section>
    </div>,
