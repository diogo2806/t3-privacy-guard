    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section><h3>Rules and permissions</h3><p>Application authentication does not imply T3N authorization. The agent needs active delegation for the exact functions, scopes and host. DENY/REDACT cannot be authorized. Privileged execution requires service authentication plus the one-time capability. Private references are enumerated. Completion additionally requires independent read-back.</p></section>
          <section><h3>Main flow</h3><p>1. Sign in. 2. Confirm tenant, agent, contract and delegation. 3. Submit attack prompt and inspect the model proposal. 4. Observe T3N DENY/REDACT/ALLOW. 5. Prepare an allowed remediation. 6. Authorize it explicitly. 7. Execute once; the action is durably claimed first. 8. Observe accepted/pending verification rather than assuming success from 2xx. 9. Verify external state until VERIFIED or leave the action UNVERIFIED for investigation. 10. Review Evidence and audit history.</p></section>
          <section><h3>Messages and errors</h3><p>Provider or T3N failures fail closed. A 401 means authentication is missing or credentials are invalid; 403 means authorization/CSRF/capability does not allow an action; 429 means sign-in is temporarily rate limited and the client must respect <strong>Retry-After</strong>; replay is rejected as conflict. An ambiguous execution is shown as UNVERIFIED and is not automatically retried. Error messages never expose provider keys, T3N keys, profile values, passwords, request/response bodies or raw headers.</p></section>
        </div>
      </section>
    </div>,
    document.body,
