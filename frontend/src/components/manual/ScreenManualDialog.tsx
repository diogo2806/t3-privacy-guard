      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p><strong>Enterprise integration UNKNOWN</strong> remains fail-closed and includes only an allowlisted diagnostic category: <code>POLICY_UNAVAILABLE</code>, <code>POLICY_INVALID</code>, <code>PRIVATE_CONFIGURATION_UNAVAILABLE</code>, <code>ENDPOINT_CONFIGURATION_INVALID</code>, <code>DELEGATION_UNAVAILABLE</code>, or <code>T3N_CONTROL_PLANE_UNAVAILABLE</code>. Facts that were not observed remain <strong>Unknown</strong> across gateway, backend, and browser instead of being converted to <strong>No</strong> or <strong>Not configured</strong>. A confirmed false value remains <strong>No</strong>, and a confirmed missing endpoint remains <strong>Not configured</strong>. Authorization, evaluation, and protected remediation still fail closed whenever any required fact is unknown. The diagnostic identifies which protected-readiness boundary could not be confirmed; it never includes a raw SDK message, machine credential, full private URL, path, query, fragment, or stack trace. <strong>NONE</strong> is used when no failure diagnostic applies.</p>
