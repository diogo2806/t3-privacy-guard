    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          <section>
            <h3>Finalidade</h3>
            <p id="screen-manual-purpose">Esta tela demonstra resposta a incidentes com separação explícita entre proposta de IA, valores operacionais confiáveis, identidade T3N, onboarding público, A2A de avaliação, Member Delegation, autorização T3N efetiva, prontidão de integração empresarial, autorização humana e execução por um Protected Executor separado. A hierarquia judge-first oferece três níveis: <strong>Protection flow</strong> para operação, <strong>Executive demo</strong> para comunicação compacta do mesmo estado runtime e <strong>Evidence</strong> para prova técnica detalhada. A IA escolhe ação e nomes de campos, mas não fornece os valores normais executados; esses valores sintéticos são gerados e persistidos pelo backend confiável e só chegam ao egress após minimização T3N.</p>
          </section>
          <section>
            <h3>Áreas, campos e ações</h3>
            <p><strong>Protection flow</strong> contém Business Outcome, cenário, prompt, proposta, decisão, próxima ação permitida, autorização humana, execução, retenção e activity rail. O <strong>Trust Flow</strong> resume AI, Policy, Human, Execute e Verify. <strong>Business Outcome</strong> traduz o mesmo estado em risco, ativo protegido, controle e resultado observado sem criar uma segunda state machine. <strong>Enterprise integration</strong> mostra se endpoint de execução, verificação independente, policy ativa e delegation do Executor formam uma configuração coerente, sem executar egress. <strong>Executive demo</strong> reúne risco, resultado observado, Trust path e Proof at a glance em uma apresentação compacta e somente leitura. Na remediação, <strong>Requested fields</strong> mostra o que o modelo solicitou; <strong>Allowed for egress</strong> mostra o subconjunto autorizado; <strong>Removed before egress</strong> mostra o que foi descartado; <strong>Trusted synthetic values</strong> são valores criados pelo backend; e <strong>Protected egress payload</strong> mostra somente os valores elegíveis para serialização. Após uma aprovação válida, <strong>Authorized by</strong> identifica a conta autenticada da aplicação que aprovou e <strong>Authorized at</strong> mostra o instante persistido dessa aprovação. <strong>Evidence</strong> mantém summary, outcomes e provenance técnica.</p>
          </section>
          <section>
            <h3>Executive Demo</h3>
            <p>A aba <strong>Executive demo</strong> é uma projeção do mesmo estado já carregado pelo dashboard. Ela não cria incidente, decisão, autorização, capability, execução, verificação ou evidence paralela. O bloco <strong>Business risk</strong> usa apenas a metadata de apresentação do cenário selecionado. <strong>Observed outcome</strong> deriva Action Proposal, Policy Decision e Remediation Execution observados. <strong>Trust path</strong> resume AI proposal, T3N policy, Human, Executor e Verify sem antecipar etapas futuras.</p>
            <p>Os estados são deliberadamente conservadores. Antes de análise aparece <strong>NOT YET OBSERVED</strong>. <strong>DENY</strong> resulta em <strong>BLOCKED BEFORE PROTECTED EGRESS</strong>; <strong>REDACT</strong> em <strong>MINIMIZATION REQUIRED</strong>; <strong>ALLOW</strong> sem aprovação em <strong>HUMAN AUTHORIZATION REQUIRED</strong>; aprovação sem side effect em <strong>AUTHORIZED / NOT EXECUTED</strong> e <strong>NOT VERIFIED YET</strong>; <strong>PENDING_VERIFICATION</strong> em <strong>ACCEPTED / NOT VERIFIED</strong>; <strong>UNVERIFIED</strong> e <strong>FAILED</strong> nunca são apresentados como sucesso. Somente <strong>COMPLETED</strong> após read-back independente permite <strong>VERIFIED EXTERNAL STATE</strong>.</p>
            <p><strong>Proof at a glance</strong> não substitui o Evidence Center. Source commit, source tree, network, contract ID/version e totais PASS/FAIL/NOT RUN só aparecem como proof quando um bundle T3N testnet válido foi carregado. Sem bundle, a tela informa <strong>Live evidence not loaded/generated</strong> e não fabrica LIVE, PASS ou commit. <strong>T3N LIVE / READY</strong> exige simultaneamente protected remediation ready, <strong>Enterprise integration READY</strong>, evidence T3N válida e zero FAIL. Proposal DID e Executor DID são encurtadas apenas visualmente; o valor completo permanece no atributo de detalhe. <strong>SEPARATE</strong>, <strong>CONFIRMED</strong>, <strong>READY</strong> e <strong>REGISTERED</strong> só aparecem quando os estados reais permitem essa afirmação.</p>
            <p>A Executive Demo não possui botões de Analyze, Authorize, Execute ou Verify. <strong>Open technical evidence</strong> apenas muda para a aba Evidence e não dispara ação protegida. O botão <strong>Manual da Tela</strong> abre este diálogo. Para alterar o estado do fluxo, use exclusivamente os controles da aba Protection flow e respeite as permissões já exigidas pelo backend.</p>
          </section>
          <section>
          ...
          <section>
            <h3>Readiness</h3>
            <p><strong>Proposal evaluation</strong> fica pronta quando Tenant, Proposal Agent e contrato estão resolvidos, o Proposal Member grant está ACTIVE e seu Effective T3N access está Confirmed. <strong>Protected remediation</strong> é um estado estritamente T3N: exige adicionalmente Protected Executor autenticado, Member grant ACTIVE e Effective T3N access Confirmed. Ele não afirma que um sistema empresarial externo esteja configurado.</p>
            <p><strong>Enterprise integration</strong> é calculada separadamente, por leitura somente, sem egress. <strong>READY</strong> exige endpoint HTTPS de execução configurado, endpoint independente de verificação configurado, presença da credencial protegida, hosts compatíveis com a policy ativa e com a delegation do Executor, além de um contrato de ação executável + verificável conhecido. <strong>INCOMPLETE</strong> significa configuração obrigatória ausente ou destino de demonstração. <strong>MISMATCH</strong> significa que configuração, policy e delegation não concordam. <strong>UNKNOWN</strong> significa que a configuração protegida não pôde ser lida ou interpretada com segurança. Nenhum desses estados prova reachability, saúde, SLA ou sucesso do serviço externo.</p>
            <p>Na implementação atual, <code>revoke-credential → REVOKED</code> é o contrato executável + verificável confirmado. <code>create-incident</code>, <code>isolate-account</code> e <code>notify-security</code> aparecem como <strong>Evaluation only</strong> enquanto não possuírem executor e read-back compatíveis. O fluxo operacional protegido só fica <strong>READY</strong> quando T3N protected remediation e Enterprise integration estão ambos prontos. A Executive Demo ainda exige evidence T3N válida com zero FAIL antes de exibir <strong>T3N LIVE / READY</strong>.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p><strong>Human authorization</strong> registra o principal autenticado da própria aplicação, não uma identidade civil nem uma DID T3N do humano. <strong>Authorized by</strong> exibe essa conta somente na área autenticada e <strong>Authorized at</strong> é o timestamp persistido da primeira aprovação válida. Retry pelo mesmo operador mantém a provenance original; outro operador não pode sobrescrevê-la silenciosamente. A capability não envia o username: ela vincula somente o SHA-256 do principal canônico e o timestamp persistido, junto do action/decision/policy/payload/destination/Protected Executor. Autorizações legadas sem esse binding aparecem como <strong>REAUTHORIZATION REQUIRED</strong> e não executam até nova autorização explícita.</p>
          ...
          <section>
            ...
            <p>Incidentes recebem <strong>expiresAt</strong> controlado pelo servidor e deixam de ser retornados após expiração. O purge remove os registros dependentes, inclusive os metadados <strong>Authorized by/Authorized at</strong>, eventos de audit e o chain-head autenticado. <strong>Execution Trace</strong> e <strong>Audit Trail</strong> ficam na lateral em desktop e abaixo do fluxo em mobile. <strong>Trace ID</strong> identifica uma tentativa HTTP e <strong>Request ID</strong> identifica a operação lógica idempotente. O trace guarda somente metadados limitados.</p>
          </section>
          <section>
            <h3>Integridade do audit local e T3N Activity Log</h3>
            <p>O audit local registra eventos de negócio sanitizados em uma cadeia <strong>HMAC-SHA256</strong> por incidente. Cada evento autenticado contém sequência monotônica, MAC do evento anterior, MAC próprio, versão/key id e, quando aplicável, a sequence/hash/function de provenance T3N. O evento <strong>HUMAN AUTHORIZATION</strong> inclui o principal autenticado sanitizado no payload local, de modo que essa provenance também fica coberta pelo HMAC. Um chain-head também autenticado permite detectar alteração de conteúdo, reordenação, lacunas e remoção da cauda no estado retido.</p>
          ...
          <section>
            ...
            <p>A evidence de autorização registra somente Member state, Effective state e funções/scopes efetivamente consultados para Proposal e Executor. Username, e-mail ou hash do operador humano não são publicados no Agent Card, evidence bundle ou T3N Activity Log. Não se persiste grant completo, <code>satisfied</code>, <code>missing</code>, tokens, credenciais ou payload operacional bruto. O live orchestrator falha antes dos cenários se qualquer principal necessário não estiver ACTIVE/Confirmed.</p>
          </section>
          <section>
            <h3>Fluxo principal</h3>
            <p>1. Entre na aplicação. 2. Use <strong>Executive demo</strong> quando precisar explicar em uma única tela risco, decisão, autoridade, outcome e proof, sem disparar ações. 3. Em <strong>Protection flow</strong>, leia T3N readiness e <strong>Enterprise integration</strong>; só considere o protected workflow pronto quando ambos estiverem prontos. 4. Leia <strong>Current result</strong> e <strong>Business Outcome</strong>. 5. Escolha o cenário e revise o prompt. 6. Use <strong>Analyze with agent</strong>. 7. Volte ao Business Outcome para conferir risco/controle observado. 8. Inspecione <strong>Requested fields</strong>, decisão e <strong>Approved destination</strong>. 9. Compare <strong>Allowed for egress</strong> com <strong>Removed before egress</strong>. 10. Confira <strong>Trusted synthetic values</strong> e <strong>Protected egress payload</strong>. 11. Em ALLOW ou REDACT executável, registre autorização humana quando exigida e confirme <strong>Authorized by/Authorized at</strong>. 12. Execute pelo Protected Executor. 13. Verifique o estado externo e confirme que Business Outcome e Executive Demo só mostram resultado verificado após read-back. 14. Em Evidence, leia summary e outcomes antes da provenance técnica.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Falha do provider na primeira ou na segunda proposta não cria Action Proposal sintética de fallback. Falha de T3N, Agent Card, A2A, policy KV, Proposal Agent, Protected Executor, <code>checkDelegation()</code> ou trust boundary nunca é apresentada como sucesso. <code>authorised=false</code> resulta em Denied; erro ou resposta inválida resulta em Unknown. Na integração empresarial, configuração ausente ou endpoint de demonstração resulta em <strong>INCOMPLETE</strong>; divergência entre host, policy e delegation resulta em <strong>MISMATCH</strong>; falha de leitura, URL protegida inválida ou dado inconclusivo resulta em <strong>UNKNOWN</strong>. O status não realiza chamada externa, portanto nunca usa a expressão endpoint healthy. Uma segunda proposta DENY, REDACT não executável ou de ação sem executor protegido permanece visível e bloqueada. <strong>REDACT</strong> só é executável quando o mínimo obrigatório sobrevive e o subconjunto reavaliado vira ALLOW. Payload confiável ausente, chave não solicitada, campo secreto ou valor alterado após autorização falha fechado. <strong>REAUTHORIZATION REQUIRED</strong> significa que um status legado não possui principal/timestamp vinculados e exige nova aprovação explícita. Tentativa de substituir silenciosamente o operador original é rejeitada. <strong>Destination changed</strong> exige nova ação, avaliação e autorização. Aceitação sem read-back permanece pendente ou UNVERIFIED. <strong>Audit integrity broken</strong> bloqueia mudanças protegidas. Prompt rejeitado informa somente a categoria detectada, sem ecoar valor ou offset. Sessão expirada exige novo login. Evidence 404 permanece como <strong>Live evidence not loaded/generated</strong> na Executive Demo, sem retry automático infinito e sem badge LIVE.</p>
          </section>
        </div>
      </section>
    </div>,
