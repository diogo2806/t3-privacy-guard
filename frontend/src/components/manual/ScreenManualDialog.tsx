      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p>Os valores em <strong>Trusted synthetic values</strong> vêm do backend confiável e são persistidos junto da ação. Após a autorização humana, o backend emite na execução uma prova <strong>v2</strong> one-time assinada com Ed25519. Ela vincula <code>normalPayloadHash</code>, action/resource/purpose, campos, referências privadas, policy version/hash, Protected Executor DID, destino aprovado, hash do principal autenticado, timestamp da autorização, issued/expiry e nonce. A chave privada permanece no backend; gateway e contrato recebem somente material público de verificação. Não há downgrade silencioso para a capability HMAC legada.</p>
            <p><strong>Human authorization</strong> registra o principal autenticado da própria aplicação, não uma identidade civil nem uma DID T3N do humano. <strong>Authorized by</strong> exibe essa conta somente na área autenticada e <strong>Authorized at</strong> é o timestamp persistido da primeira aprovação válida. Retry pelo mesmo operador mantém a provenance original; outro operador não pode sobrescrevê-la silenciosamente. A prova não envia o username: ela vincula somente o SHA-256 do principal canônico e o timestamp persistido. Autorizações legadas sem esse binding aparecem como <strong>REAUTHORIZATION REQUIRED</strong> e não executam até nova autorização explícita.</p>
            <p><strong>Gateway proof check</strong> é um fail-fast e não substitui a proteção T3N. O gateway encaminha a mesma prova assinada ao <code>execute-remediation</code>. Antes de ler <code>security_api_url</code>, <code>security_api_key</code> ou iniciar HTTP, o WASM verifica assinatura, key id, expiração, todos os claims e consome o nonce em KV privado do contrato. Uma chamada direta com a credencial do Protected Executor mas sem prova, com prova expirada/tampered ou com nonce já consumido falha fechado. A tela não afirma que T3N verificou a identidade civil do humano; o que T3N verifica é a prova assinada emitida pelo backend após autenticação e autorização.</p>
            <p><strong>Policy-allowed destination</strong> significa que um hostname pertence ao conjunto permitido pela policy. <strong>Approved destination</strong> é o hostname exato vinculado à autorização humana. A URL completa permanece no KV privado. <strong>Execute protected credential revocation</strong> exige prova válida + Executor autorizado, correspondência do hostname, policy atual compatível, payload minimizado e reavaliação ALLOW antes do endpoint externo. Aceitação externa não é conclusão; <strong>COMPLETED</strong> só aparece após read-back independente do estado esperado.</p>
          </section>
          <section>
