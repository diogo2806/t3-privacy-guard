      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p>Os valores em <strong>Trusted synthetic values</strong> vêm do backend confiável e são persistidos junto da ação. A autorização humana produz, somente no momento da execução, um <strong>one-time authorization proof v2</strong> assinado com Ed25519. A chave privada permanece no backend; gateway e contrato T3N recebem somente a chave pública. O proof vincula <code>normalPayloadHash</code>, action/resource/purpose, campos, referências privadas, policy version/hash, Protected Executor DID, <strong>Approved destination</strong>, SHA-256 do principal autenticado, timestamp persistido, emissão, expiração, nonce e <code>keyId</code>. Alterar qualquer binding invalida a prova.</p>
            <p>O gateway verifica a assinatura e os bindings como fail-fast, mas não é a última barreira. O mesmo proof chega ao <code>execute-remediation</code>; antes de ler URL/chave de egress ou chamar HTTP, o WASM verifica Ed25519, TTL, policy/action/request/destination/payload, Protected Executor DID e provenance humana assinada, e consome o hash do nonce no KV privado T3N. Credencial do Executor sem proof válido não basta. Proof ausente, expirado, alterado, repetido ou em formato legado falha fechado. A rotação de chave mantém um <code>keyId</code> ativo e só aceita chave pública anterior dentro de uma janela explícita e limitada no KV T3N. O indicador <strong>T3N execution proof check</strong> só aparece como VERIFIED após uma execução que atravessou esse boundary.</p>
            <p><strong>Human authorization</strong> registra o principal autenticado da própria aplicação, não uma identidade civil nem uma DID T3N do humano. <strong>Authorized by</strong> exibe essa conta somente na área autenticada e <strong>Authorized at</strong> é o timestamp persistido da primeira aprovação válida. Retry pelo mesmo operador mantém a provenance original; outro operador não pode sobrescrevê-la silenciosamente. A prova não envia o username ao T3N: ela vincula somente o SHA-256 do principal canônico e o timestamp persistido. Autorizações legadas sem esse binding aparecem como <strong>REAUTHORIZATION REQUIRED</strong> e não executam até nova autorização explícita.</p>
            <p><strong>Policy-allowed destination</strong> significa que um hostname pertence ao conjunto permitido pela policy. <strong>Approved destination</strong> é o hostname exato vinculado à autorização humana. A URL completa permanece no KV privado. <strong>Execute protected credential revocation</strong> exige proof válido, correspondência do hostname, policy atual compatível, <strong>Protected egress payload</strong> formado somente pela interseção entre valores persistidos e <code>allowed_fields</code>, reavaliação do conjunto mínimo e só então chama o endpoint externo. Aceitação externa não é conclusão; <strong>COMPLETED</strong> só aparece após read-back independente do estado esperado.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p><strong>PASS</strong> significa resultado observado compatível com o esperado. <strong>FAIL</strong> é divergência observada. <strong>NOT RUN</strong> nunca conta como prova nem como PASS. <code>LIVE-AI-MINIMUM-REMEDIATION</code> só passa quando o provider/model configurado realmente produz a proposta mínima esperada e essa saída exata recebe T3N ALLOW; com AI desabilitada permanece NOT RUN. <code>LIVE-EXECUTOR-REQUIRES-HUMAN-PROOF</code> só passa quando o Executor está ACTIVE/Confirmed e uma chamada direta de <code>execute-remediation</code> sem proof é rejeitada pelo T3N/WASM. O cenário opcional <code>LIVE-NORMAL-PAYLOAD-MINIMIZATION</code> é testnet-only e passa somente quando um read-back controlado informa <code>must_egress_seen=true</code> e <code>must_not_egress_seen=false</code>. Os sentinelas brutos não retornam no bundle. <strong>CLEAN</strong> é sinal de reprodutibilidade do working tree, não auditoria independente.</p>
            <p>A evidence de autorização registra somente Member state, Effective state, funções/scopes consultados e, no cenário de proof, fingerprint SHA-256 da chave pública de verificação. Username, e-mail, hash do operador, proof completo, nonce bruto ou chave privada não são publicados no Agent Card, evidence bundle ou T3N Activity Log. Não se persiste grant completo, <code>satisfied</code>, <code>missing</code>, tokens, credenciais ou payload operacional bruto. O live orchestrator falha antes dos cenários se qualquer principal necessário não estiver ACTIVE/Confirmed.</p>
          </section>
          <section>
            <h3>Fluxo principal</h3>
            <p>1. Entre na aplicação. 2. Leia readiness, <strong>Current result</strong> e <strong>Business Outcome</strong>. 3. Escolha o cenário e revise o prompt. 4. Use <strong>Analyze with agent</strong>. 5. Volte ao Business Outcome para conferir risco/controle observado. 6. Inspecione <strong>Requested fields</strong>, decisão e <strong>Approved destination</strong>. 7. Compare <strong>Allowed for egress</strong> com <strong>Removed before egress</strong>. 8. Confira <strong>Trusted synthetic values</strong> e <strong>Protected egress payload</strong>. 9. Em ALLOW ou REDACT executável, registre autorização humana quando exigida e confirme <strong>Authorized by/Authorized at</strong>. 10. Execute pelo Protected Executor e confira o estado do one-time proof/T3N proof check. 11. Verifique o estado externo e confirme que Business Outcome só mostra resultado verificado após read-back. 12. Em Evidence, leia summary e outcomes antes da provenance técnica.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Falha do provider na primeira ou na segunda proposta não cria Action Proposal sintética de fallback. Falha de T3N, Agent Card, A2A, policy KV, Proposal Agent, Protected Executor, <code>checkDelegation()</code> ou trust boundary nunca é apresentada como sucesso. <code>authorised=false</code> resulta em Denied; erro ou resposta inválida resulta em Unknown. Uma segunda proposta DENY, REDACT não executável ou de ação sem executor protegido permanece visível e bloqueada. <strong>REDACT</strong> só é executável quando o mínimo obrigatório sobrevive e o subconjunto reavaliado vira ALLOW. Payload confiável ausente, chave não solicitada, campo secreto ou valor alterado após autorização falha fechado. <strong>REAUTHORIZATION REQUIRED</strong> significa que um status legado não possui principal/timestamp vinculados e exige nova aprovação explícita. Tentativa de substituir silenciosamente o operador original é rejeitada. Proof ausente, assinatura inválida, proof expirado, binding divergente, keyId fora da janela ativa ou nonce repetido é rejeitado antes do egress; possuir a credencial do Protected Executor não substitui autorização humana. <strong>Destination changed</strong> exige nova ação, avaliação e autorização. Aceitação sem read-back permanece pendente ou UNVERIFIED. <strong>Audit integrity broken</strong> bloqueia mudanças protegidas. Prompt rejeitado informa somente a categoria detectada, sem ecoar valor ou offset. Sessão expirada exige novo login.</p>
          </section>
        </div>
      </section>
    </div>,
