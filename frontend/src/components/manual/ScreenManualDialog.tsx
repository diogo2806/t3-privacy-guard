    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            <h3>Retenção e Execution Trace</h3>
            <p>Incidentes recebem <strong>expiresAt</strong> controlado pelo servidor e deixam de ser retornados após expiração. O purge remove os registros dependentes, inclusive eventos de audit e o chain-head autenticado; a aplicação não conserva uma âncora HMAC indefinidamente só para preservar prova. <strong>Trace ID</strong> identifica uma tentativa HTTP e <strong>Request ID</strong> identifica a operação lógica idempotente. O trace guarda somente metadados limitados.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p>1. Entre na aplicação. 2. Confira Tenant, Proposal Agent e Protected Executor. 3. Diferencie Agent Card/A2A, Member grant e Effective T3N access. 4. Confirme Proposal evaluation antes de avaliar. 5. Para remediação, confirme também Protected remediation. 6. Escolha o cenário e revise o prompt. 7. Use Ask agent. 8. Inspecione proposta e decisão. 9. Confira a integridade local antes de uma mudança protegida. 10. Registre autorização humana quando aplicável. 11. Execute pelo Protected Executor. 12. Verifique o estado externo. 13. Consulte Execution Trace, integridade do audit local, provenance T3N e Proof &amp; evidence. A2A pode reproduzir somente a etapa pública de proposta/avaliação.</p>
          </section>
          <section>
