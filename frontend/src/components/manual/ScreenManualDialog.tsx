    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          <section>
            <h3>Finalidade</h3>
            <p id="screen-manual-purpose">Esta tela demonstra resposta a incidentes com separação explícita entre proposta de IA, identidade T3N, onboarding público do agente, interoperabilidade A2A de avaliação, autorização por delegação, autorização humana e execução por um principal T3N privilegiado separado. A IA ou um cliente A2A pode solicitar avaliação, mas não pode conceder permissão a si mesmo, obter a credencial do Protected Executor, alterar a policy ativa, obter valores privados diretamente nem declarar uma remediação crítica como concluída.</p>
          </section>
          <section>
            <h3>Áreas da tela</h3>
          ...
          <section>
            ...
            <p>O card público sempre anuncia o serviço <strong>DID</strong> suportado. Quando <code>A2A_PUBLIC_URL</code> está configurado com uma URL HTTPS pública válida, ele também anuncia o serviço <strong>A2A</strong> apontando para o discovery correspondente. O card não contém API key, private key, token, endpoint interno, MCP ou x402. Os estados <strong>NOT REGISTERED</strong>, <strong>CARD/DID MISMATCH</strong> e <strong>UNAVAILABLE</strong> nunca são apresentados como sucesso. Um card divergente ou inválido não é aceito apenas por responder HTTP 200.</p>
          </section>
          <section>
            <h3>A2A público</h3>
            <p><strong>A2A configured</strong> significa apenas que existe uma <code>A2A_PUBLIC_URL</code> HTTPS pública válida na configuração do gateway. <strong>A2A published</strong> significa que o Agent Card T3N resolvido publicou o serviço A2A esperado. Esses estados não provam que o endpoint foi alcançado por um cliente externo; reachability live exige teste independente. O endpoint público aceita somente A2A v1.0 por JSON-RPC <code>SendMessage</code>, uma mensagem textual limitada e o header oficial <code>A2A-Version</code>. Campos extras, identidade controlada pelo cliente, <code>decision</code>, <code>agent_did</code>, <code>pii_did</code>, capabilities, payload oversized e prompts sensíveis suportados são rejeitados antes de qualquer autorização privilegiada.</p>
            <p>O A2A reutiliza o mesmo fluxo de proposta do agente e <code>evaluate-action</code>. A resposta pode conter proposta estruturada e provenance mínima da decisão T3N, mas não expõe capability, credencial, autorização humana, <code>execute-remediation</code>, <code>verify-remediation</code> ou o Protected Executor. Um resultado <strong>ALLOW</strong> recebido por A2A continua sendo apenas decisão de policy e nunca equivale a execução.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p><strong>PASS</strong> significa resultado observado compatível com o esperado. <strong>FAIL</strong> é divergência observada. <strong>NOT RUN</strong> não conta como prova. O bundle identifica o SHA completo do <strong>Source commit</strong> e o estado <strong>Source tree</strong> como CLEAN ou DIRTY, além de rede, SDK, tenant DID, Proposal Agent DID, Protected Executor DID, Agent Card URI/hash/status, serviços observados no card, contrato, WASM SHA-256, policy version/hash e trust-manifest provenance. O source commit liga a execução à revisão pública do código; ele não substitui WASM/policy hashes nem significa auditoria independente. CLEAN informa apenas que a árvore Git estava sem alterações quando a geração começou; DIRTY permanece explícito e não é promovido a sucesso. <strong>A2A OBSERVED</strong> significa apenas que o Agent Card resolvido anunciou o serviço A2A durante a geração do bundle; não prova reachability do endpoint. O cenário <strong>LIVE-PROPOSAL-CANNOT-EXECUTE</strong>, quando executado, deve provar que a própria Member Delegation T3N rejeita tentativa do Proposal Agent de chamar a função privilegiada. Agent Card prova onboarding público, não autorização nem attestation de hardware.</p>
          </section>
          <section>
            <h3>Permissões e regras</h3>
            <p>Login na aplicação não concede autoridade T3N. As DIDs do Tenant, Proposal Agent e Protected Executor vêm de sessões autenticadas, nunca de valores hardcoded. O Agent Card e o endpoint A2A não concedem funções. Member grant observado não equivale sozinho a effective access. O Proposal Agent só é tratado como autorizado quando sua própria sessão passa em <code>checkDelegation()</code>; o mesmo vale para o Protected Executor. A Proposal delegation não pode conter funções de remediação; a Executor delegation não deve ser usada pelo fluxo de proposta. A2A aceita avaliação somente e não fornece um caminho alternativo para autorização humana ou execução. Rotas internas usam service token. A interface não promove DENY/REDACT para execução, não inventa receipt T3N e não transforma indisponibilidade em sucesso.</p>
          </section>
          <section>
            <h3>Fluxo principal</h3>
            <p>1. Entre na aplicação. 2. Confira status de Tenant, Proposal Agent e Protected Executor. 3. Diferencie Agent Card, A2A configurado/publicado, Member grant, Platform delegation e Effective access. 4. Confirme que os dois effective access estão ACTIVE antes de interpretar o control plane como pronto. 5. Escolha um cenário e revise o prompt. 6. Use Ask agent. 7. Inspecione a proposta real. 8. Leia a decisão T3N e sua policy version/hash. 9. Se houver ALLOW para revogação suportada, registre autorização humana. 10. Confirme que a autorização está vinculada ao Protected Executor. 11. Execute uma vez. 12. Verifique o estado externo. 13. Consulte Execution Trace, audit provenance e Proof &amp; evidence; no bundle live, confira Source commit, Source tree, WASM SHA-256 e Policy SHA-256 como identidades distintas. A2A pode reproduzir somente a etapa de proposta + avaliação, nunca as etapas 9 a 12.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Quando uma classe suportada é detectada, o prompt é rejeitado antes do provider remoto e a resposta não ecoa o valor ou offset detectado. Remova o literal e use contexto não sensível ou uma referência lógica aprovada quando existir. Falha de provedor, T3N, Agent Card, policy KV, Proposal Agent, Protected Executor, <code>checkDelegation()</code> ou trust boundary não é apresentada como sucesso. Card ausente gera <strong>NOT REGISTERED</strong>; divergência de DID/schema/serviços gera <strong>CARD/DID MISMATCH</strong>; indisponibilidade de resolução gera <strong>UNAVAILABLE</strong>. A2A configurado sem publicação observada não é mostrado como publicado, e publicação observada não é mostrada como live-tested. Requisição A2A inválida, versão ausente/incorreta, rate limit, prompt sensível e body oversized retornam erro sanitizado sem expor segredos. Member grant <strong>SCHEDULED</strong>, <strong>REVOKED</strong> ou <strong>NOT GRANTED</strong> permanece não operacional. <code>authorised=false</code> resulta em effective access <strong>INCOMPLETE</strong>; erro ou resposta inválida resulta em <strong>UNKNOWN</strong>. Troca da Executor DID entre autorização e execução invalida a capability. Falha no Activity Log mantém o audit local disponível e marca a provenance de rede como não verificada. Mudança de policy após autorização bloqueia execução protegida. Sessão expirada exige novo login.</p>
          </section>
        </div>
      </section>
    </div>,
