      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p><strong>Risk</strong> e <strong>Protected asset</strong> são metadata de apresentação do cenário, não decisões de segurança. <strong>Policy decision</strong>, contagens de field names permitidos/redacted, destino, autorização, execução, verification attempts e final state vêm do estado observado. O Business Outcome continua tratando as contagens como <strong>field names</strong>; a prova de minimização por valor aparece no painel de remediação e na evidence específica. <strong>Approved destination</strong> só é apresentado como autorizado quando o status persistido da ação comprova autorização. <strong>Authorized by</strong>/<strong>Authorized at</strong> só aparecem depois de uma autorização persistida; estado antigo sem provenance é <strong>LEGACY UNBOUND</strong>, não identidade inventada.</p>
          ...
          <section>
            <h3>Operador autenticado e provenance da autorização humana</h3>
            <p><strong>Authorized by</strong> identifica exclusivamente a conta autenticada da aplicação que aprovou a remediação. Esse principal vem do <code>SecurityContext</code> do servidor e nunca de um campo enviado pelo browser. Ele não é a DID do Proposal Agent, não é a DID do Protected Executor e não comprova identidade civil do humano.</p>
            <p>Na primeira autorização válida, o backend persiste o principal canônico e <strong>Authorized at</strong>. Retry pelo mesmo principal preserva o timestamp original; outro principal não pode substituir silenciosamente a aprovação. O evento <strong>REMEDIATION_AUTHORIZED</strong> registra a provenance de forma sanitizada dentro da cadeia HMAC local. A retenção desses metadados acompanha a própria ação/incidente.</p>
            <p>A one-time capability não transporta o username: ela contém <code>operatorPrincipalHash = SHA-256(UTF8(canonical principal))</code> e o timestamp persistido <code>authorizedAt</code>, além de um <code>issuedAt</code> separado. O gateway exige igualdade exata entre capability e request interno e valida <code>authorizedAt &lt;= issuedAt</code>. O hash é provenance vinculada, não um mecanismo de login e não substitui autenticação.</p>
            <p>Uma ação antiga em <strong>REMEDIATION_AUTHORIZED</strong> sem principal/timestamp aparece como <strong>LEGACY UNBOUND</strong>. A execução fica bloqueada até uma reautorização explícita que vincule o operador atual. Uma ação já <strong>REMEDIATED</strong> sem provenance não é retroativamente atribuída a alguém. Username/e-mail do operador não entra em Agent Card, bundle público de evidence ou T3N Activity Log.</p>
          </section>
          <section>
            <h3>Readiness</h3>
          ...
          <section>
            ...
            <p>Os valores em <strong>Trusted synthetic values</strong> vêm do backend confiável e são persistidos junto da ação. A capability humana assina <code>normalPayloadHash</code> sobre um formato canônico do mapa, além de action/resource/purpose, campos, referências privadas, policy version/hash, Protected Executor DID, <strong>Approved destination</strong>, hash do principal humano autenticado e timestamp persistido da aprovação. Alterar qualquer binding depois da autorização invalida a capability. Campo secreto, chave não solicitada, payload confiável ausente ou mínimo obrigatório removido falham fechado antes do protected HTTP.</p>
            <p><strong>Policy-allowed destination</strong> significa que um hostname pertence ao conjunto permitido pela policy. <strong>Approved destination</strong> é o hostname exato vinculado à autorização humana. A URL completa permanece no KV privado. <strong>Execute protected credential revocation</strong> exige provenance humana vinculada, correspondência do hostname, revalida a policy atual, monta o <strong>Protected egress payload</strong> somente com a interseção entre valores persistidos e <code>allowed_fields</code>, reavalia o conjunto mínimo e só então chama o endpoint externo. Aceitação externa não é conclusão; <strong>COMPLETED</strong> só aparece após read-back independente do estado esperado.</p>
          </section>
          <section>
            <h3>Retenção e activity rail</h3>
            <p>Incidentes recebem <strong>expiresAt</strong> controlado pelo servidor e deixam de ser retornados após expiração. O purge remove os registros dependentes, inclusive metadata da autorização humana, eventos de audit e o chain-head autenticado. <strong>Execution Trace</strong> e <strong>Audit Trail</strong> ficam na lateral em desktop e abaixo do fluxo em mobile. <strong>Trace ID</strong> identifica uma tentativa HTTP e <strong>Request ID</strong> identifica a operação lógica idempotente. O trace guarda somente metadados limitados.</p>
          </section>
          <section>
            <h3>Integridade do audit local e T3N Activity Log</h3>
            <p>O audit local registra eventos de negócio sanitizados em uma cadeia <strong>HMAC-SHA256</strong> por incidente. Cada evento autenticado contém sequência monotônica, MAC do evento anterior, MAC próprio, versão/key id e, quando aplicável, a sequence/hash/function de provenance T3N. O evento de autorização inclui a conta da aplicação de forma sanitizada; como faz parte da mensagem autenticada, alterá-la também quebra a verificação da cadeia. Um chain-head também autenticado permite detectar alteração de conteúdo, reordenação, lacunas e remoção da cauda no estado retido.</p>
