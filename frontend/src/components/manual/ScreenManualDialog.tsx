    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section><h3>Authority separation</h3><p>The AI provider never receives T3N tenant/agent keys, remediation credentials, service tokens, one-time capabilities or resolved profile values. Agent DID and tenant DID come from authenticated T3N sessions. The model cannot select identities, contract, policy decision or human authorization proof.</p></section>
          ...
          <section><h3>Audit provenance</h3><p><strong>Local business audit</strong> contains application events persisted by the incident workflow. <strong>T3N Activity Log</strong> contains sanitized metadata read from the official T3N Activity Log through the authenticated gateway; the browser never talks directly to T3N. <strong>Matched</strong> is shown only when sequence, hash, canonical contract, agent and function agree exactly. <strong>Unmatched</strong> means a local event expected T3N provenance but no exact reference was verified; it is an evidence state, not a business failure. <strong>Local only</strong> is expected for application-only events. <strong>T3N only</strong> means a relevant network event exists without a bound local audit event. <strong>Refresh provenance</strong> is read-only and never repeats policy evaluation or remediation. The activity view is bounded and ordered by T3N sequence rather than timestamp. If T3N activity is unavailable, the local audit remains visible and the UI states that network provenance was not verified. Raw call payloads, secrets and private values are never displayed or persisted as Activity Log evidence.</p></section>
          ...
          <section><h3>Evidence states</h3><p><strong>PASS</strong> means observed result matched expectation. <strong>FAIL</strong> means it did not. <strong>NOT RUN</strong> means a scenario was not executed and is never counted as PASS. Profile resolution or external verification is not claimed live unless matching testnet evidence exists.</p></section>
          <section><h3>Rules and permissions</h3><p>Application authentication does not imply T3N authorization. The agent needs active delegation for the exact functions, scopes and host. DENY/REDACT cannot be authorized. Privileged execution requires service authentication plus the one-time capability. Private references are enumerated. Completion additionally requires independent read-back. Activity Log reads are read-only, bounded, service-authenticated between Spring and the gateway, and never change policy or remediation state.</p></section>
          <section><h3>Main flow</h3><p>1. Sign in. 2. Confirm tenant, agent, contract and delegation. 3. Enter only non-sensitive prompt text or logical private references. 4. Submit the attack prompt and inspect the model proposal. 5. Observe T3N DENY/REDACT/ALLOW. 6. Prepare an allowed remediation. 7. Authorize it explicitly. 8. Execute once; the action is durably claimed first. 9. Observe accepted/pending verification rather than assuming success from 2xx. 10. Verify external state until VERIFIED or leave the action UNVERIFIED for investigation. 11. Review Local business audit and T3N Activity Log provenance. 12. Review the separate Evidence view for submission evidence.</p></section>
          <section><h3>Messages and errors</h3><p>A sensitive prompt is rejected with HTTP 422 before the external provider is called; the response never echoes the detected value. Provider or T3N failures fail closed. A 401 means authentication is missing or credentials are invalid; 403 means authorization/CSRF/capability does not allow an action; 429 means sign-in is temporarily rate limited and the client must respect <strong>Retry-After</strong>; replay is rejected as conflict. If Activity Log retrieval is unavailable, the local audit remains available and network provenance is explicitly marked as not verified. A bounded or truncated activity window never turns an unmatched item into proof of absence. An ambiguous execution is shown as UNVERIFIED and is not automatically retried. Error messages never expose provider keys, T3N keys, profile values, passwords, request/response bodies, Activity Log raw payloads or raw headers.</p></section>
        </div>
      </section>
    </div>,
    document.body,
