      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p><strong>Local business audit</strong> contains application events persisted by the incident workflow. <strong>T3N Activity Log</strong> contains sanitized network metadata read by the authenticated gateway; the browser never talks directly to T3N. <strong>Matched</strong> requires exact sequence, hash, canonical contract, agent and function identifiers. <strong>Unmatched</strong> means a local event expected T3N provenance but no exact reference was verified; it is an evidence state, not a business failure. <strong>Local only</strong> is expected for application-only events. <strong>T3N only</strong> means a relevant network event exists without a bound local event. <strong>Refresh provenance</strong> is read-only and never repeats policy evaluation or remediation. The view is bounded and ordered by T3N sequence, and raw payloads, secrets and private values are never displayed or persisted as Activity Log evidence.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p>Application sign-in does not grant T3N authority. Policy must allow the exact action and scope, critical remediation requires human authorization, protected execution requires its runtime controls, and completion additionally requires independent read-back. DENY and REDACT cannot be promoted to execution by the interface. Activity Log reads are read-only, bounded and service-authenticated between Spring and the gateway; they never alter policy or remediation state.</p>
          </section>
          <section>
            <h3>Main flow</h3>
            <p>1. Sign in. 2. Read the trust flow and confirm whether T3N controls are available. 3. Enter a non-sensitive prompt. 4. Inspect the proposal. 5. Observe DENY, REDACT or ALLOW. 6. For an allowed remediation, authorize it as a human operator. 7. Execute the protected action once. 8. Treat acceptance as pending verification. 9. Verify the external state. 10. Review local business audit and T3N Activity Log provenance. 11. Review Proof &amp; evidence and technical details when needed.</p>
          </section>
          <section>
            <h3>Messages and error states</h3>
            <p>Sensitive prompt content is rejected before reaching the external provider. Provider, T3N or trust-boundary failures fail closed. Expired application sessions require sign-in again. Rate-limited sign-in follows the server retry interval. If Activity Log retrieval is unavailable, the local business audit remains available and network provenance is explicitly marked as not verified. A bounded or truncated activity window never turns an unmatched item into proof of absence. Ambiguous execution remains UNVERIFIED and is not automatically re-executed. Error messages must not expose private values, passwords, credentials, request bodies, raw Activity Log payloads or raw headers.</p>
          </section>
        </div>
      </section>
    </div>,
