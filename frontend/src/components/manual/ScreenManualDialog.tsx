    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section><h3>Retry rule</h3><p>Repeating execution for the same action reconciles the durable claim instead of sending another external side effect. A retry keeps the same Request ID but receives a new Trace ID, so idempotency and observability remain separate. If a process restarts after the request may have been sent but before an operation ID is persisted, the state becomes UNVERIFIED and automatic re-execution is blocked. A manual verification is offered only when an operation ID exists.</p></section>
          ...
          <section><h3>Rules and permissions</h3><p>Application authentication does not imply T3N authorization. The agent needs active delegation for the exact functions, scopes and host. DENY/REDACT cannot be authorized. Privileged execution requires service authentication plus the one-time capability. Private references are enumerated. A Trace ID grants no authority and cannot be used to access another incident or action. Completion additionally requires independent read-back.</p></section>
          <section><h3>Main flow</h3><p>1. Sign in. 2. Confirm tenant, agent, contract and delegation. 3. Enter only non-sensitive prompt text or logical private references. 4. Submit the attack prompt and inspect the model proposal. 5. Observe T3N DENY/REDACT/ALLOW. 6. Prepare an allowed remediation. 7. Authorize it explicitly. 8. Execute once; the action is durably claimed first. 9. Observe accepted/pending verification rather than assuming success from 2xx. 10. Verify external state until VERIFIED or leave the action UNVERIFIED for investigation. 11. Review the Execution Trace for technical correlation, the Business Audit Trail for persisted business events and Evidence for reproducible T3N proof.</p></section>
          <section><h3>Messages and errors</h3><p>A sensitive prompt is rejected with HTTP 422 before the external provider is called; the response never echoes the detected value. Provider or T3N failures fail closed. A 401 means authentication is missing or credentials are invalid; 403 means authorization/CSRF/capability does not allow an action; 429 means sign-in is temporarily rate limited and the client must respect <strong>Retry-After</strong>; replay is rejected as conflict. An ambiguous execution is shown as UNVERIFIED and is not automatically retried. Error messages and trace metadata never expose provider keys, T3N keys, profile values, passwords, request/response bodies or raw headers.</p></section>
        </div>
      </section>
    </div>,
    document.body,
