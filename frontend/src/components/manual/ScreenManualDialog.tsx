      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p><strong>Registered</strong> significa que um Agent Card público válido foi resolvido para exatamente a mesma Proposal Agent DID. O Agent Card serve para descoberta pública e não concede autoridade sobre o contrato. <strong>Member grant</strong> é o registro observado no documento de delegação do data owner e representa a intenção/autorização gravada pelo Tenant para um principal específico, com funções, scopes, hosts e janela temporal.</p>
            ...
            <p>O card público sempre anuncia o serviço <strong>DID</strong> realmente suportado e anuncia <strong>A2A</strong> somente quando <code>A2A_PUBLIC_URL</code> foi configurada como endpoint HTTPS válido. O serviço A2A no card aponta para <code>/.well-known/agent-card.json</code>, que por sua vez declara o endpoint público de interação. O card não contém API key, private key, token, endpoint interno, MCP, x402 ou trust não comprovado. Os estados <strong>NOT REGISTERED</strong>, <strong>CARD/DID MISMATCH</strong> e <strong>UNAVAILABLE</strong> nunca são apresentados como sucesso.</p>
          </section>
          <section>
            <h3>A2A evaluation service</h3>
            <p>Quando habilitado, o gateway expõe <code>GET /.well-known/agent-card.json</code> e <code>POST /a2a</code> usando A2A v1.0 por JSON-RPC. A única habilidade pública é solicitar análise e decisão de policy. O pedido contém apenas uma mensagem de texto sintética; prompts com PII ou credenciais reconhecidas são rejeitados antes do provedor remoto, e payloads que tentem enviar <code>agent_did</code>, <code>pii_did</code>, <code>decision</code>, capabilities ou campos desconhecidos são rejeitados.</p>
            <p>O <strong>Agent DID</strong> usado na avaliação vem da sessão autenticada do Proposal Agent e o <strong>pii_did</strong> vem da sessão autenticada do Tenant dentro do fluxo T3N existente. O cliente A2A não escolhe essas identidades. A resposta pública contém somente a proposta estruturada, decisão, reason code, policy version/hash e Agent DID. Autorização humana, capability de remediação, credencial do Protected Executor e funções <code>execute-remediation</code>/<code>verify-remediation</code> não são expostas por A2A.</p>
            <p><strong>Configured</strong> significa apenas que a URL pública passou pela validação local. <strong>Published</strong> significa que o serviço A2A foi observado no Agent Card T3N resolvido. Nenhum desses estados é chamado de <strong>verified live</strong> sem um teste externo real do endpoint; o painel informa explicitamente quando esse teste não foi executado.</p>
          </section>
          <section>
