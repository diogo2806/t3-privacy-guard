        ...
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
          <section className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
            ...
            <div className="manual-content">
              <section><h3>Purpose</h3><p id="screen-manual-purpose">Demonstrates how T3 Privacy Guard blocks a malicious AI proposal while still allowing a minimum, human-authorized remediation without exposing the remediation credential to browser, Java or the AI agent.</p></section>
              ...
              <section><h3>Actions</h3><p><strong>Run attack scenario</strong> evaluates an exfiltration attempt. <strong>Prepare safe remediation</strong> evaluates the minimum legitimate revoke request. <strong>Authorize remediation</strong> records human business authorization but does not execute it. <strong>Execute protected remediation</strong> invokes the TEE contract. <strong>Refresh status</strong> reloads operational state. <strong>Refresh evidence</strong> only rereads sanitized evidence. <strong>Sign out</strong> invalidates the operator session.</p></section>
              <section><h3>Rules and permissions</h3><p>Application authentication does not imply T3N authorization. The agent still needs an active delegation for the exact contract function, scopes and outbound host. A DENY or REDACT decision cannot be human-authorized for remediation. UNKNOWN, NOT_GRANTED and REVOKED are never success states. Evidence APIs require the operator session and never return raw logs, `.env` files, cookies or secret values.</p></section>
              <section><h3>Main flow</h3><p>1. Sign in. 2. Confirm tenant, agent, contract and delegation separately. 3. Run attack and observe DENY. 4. Prepare safe action and observe ALLOW. 5. Record human authorization. 6. Execute protected remediation when appropriate. 7. Open Evidence and verify T3N_TESTNET metadata/results. 8. Review audit history. 9. Sign out.</p></section>
              <section><h3>Messages and errors</h3><p>A 401 means the operator session is missing or expired. A 403 means the session or CSRF protection does not allow an action. Missing evidence is an explicit empty state. Operational/evidence states that cannot be validated are never presented as verified or successful.</p></section>
            </div>
          </section>
        </div>
      )}
