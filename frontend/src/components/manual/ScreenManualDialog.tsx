      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p>Um risco apresentado não equivale a um risco comprovadamente bloqueado. <strong>DENY</strong> comprova que a proposta observada foi bloqueada pelo controle antes do protected egress. <strong>REDACT</strong> pode continuar somente quando o mínimo específico da ação permanece permitido e o subconjunto reavaliado vira ALLOW. <strong>ALLOW</strong> permite continuar, mas ainda exige autorização humana quando aplicável. <strong>PENDING_VERIFICATION</strong> significa side effect aceito, não concluído. Para revogação, sucesso verificado é <strong>REVOKED — VERIFIED</strong>. Para notificação, <strong>DELIVERED — VERIFIED</strong> só aparece após read-back independente com <code>recipient_resolved=true</code>. O Business Outcome não duplica Authorize, Execute ou Verify.</p>
          </section>
          <section>
            ...
            <p>No cenário <strong>Notify security contact</strong>, navegador, modelo, Spring e gateway trafegam apenas a referência lógica <code>verified_email</code>. O literal T3N <code>{'{{profile.verified_contacts.email.value}}'}</code> é criado somente dentro do contrato Rust/WASM. O valor resolvido pode existir transitoriamente no protected egress e no destinatário autorizado, mas não é retornado por APIs, Business Outcome, Audit Trail ou Evidence.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p><strong>DENY</strong> bloqueia. <strong>REDACT</strong> significa que a policy removeu parte do pedido. Para revogação, <code>incident_id</code>, <code>credential_id</code> e <code>reason</code> precisam permanecer em <strong>Allowed for egress</strong>. Para notificação, o mínimo normal é <code>incident_id</code>, <code>severity</code> e <code>summary</code>, e a referência privada deve continuar exatamente <code>verified_email</code>. Em ambos os casos o contrato reavalia o subconjunto e exige ALLOW antes do HTTP. <strong>Removed before egress</strong> nunca é copiado para o request externo.</p>
            ...
            <p><strong>Policy-allowed destination</strong> significa que um hostname pertence ao conjunto permitido pela policy. <strong>Approved destination</strong> é o hostname exato vinculado à autorização humana. A URL completa permanece no KV privado. <strong>Execute protected credential revocation</strong> e <strong>Execute protected security notification</strong> exigem correspondência do hostname, revalidam a policy atual e montam o egress somente com o normal payload permitido. Na notificação, o contrato adiciona o marker de <code>verified_email</code> apenas dentro do protected execution. Aceitação externa não é conclusão: revogação exige read-back <strong>REVOKED</strong>; notificação exige <strong>DELIVERED</strong> e <code>recipient_resolved=true</code>.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p><strong>PASS</strong> significa resultado observado compatível com o esperado. <strong>FAIL</strong> é divergência observada. <strong>NOT RUN</strong> nunca conta como prova nem como PASS. O cenário opcional <code>LIVE-NORMAL-PAYLOAD-MINIMIZATION</code> passa somente com read-back controlado de booleans. O cenário <code>LIVE-PROFILE-PLACEHOLDER-RESOLUTION</code> permanece NOT RUN por padrão e só pode virar PASS em testnet, com <code>EVIDENCE_RUN_PROFILE_PLACEHOLDER=true</code>, perfil sintético contendo e-mail verificado e endpoints controlados; PASS exige ALLOW → PENDING_VERIFICATION → VERIFIED DELIVERED com <code>recipient_resolved=true</code>. Recipient plaintext e marker não entram no bundle.</p>
          ...
          <section>
            ...
            <p>1. Entre na aplicação. 2. Leia readiness, <strong>Current result</strong> e <strong>Business Outcome</strong>. 3. Escolha o cenário e revise o prompt. 4. Use <strong>Analyze with agent</strong>. 5. Volte ao Business Outcome para conferir risco/controle observado. 6. Inspecione <strong>Requested fields</strong>, decisão e <strong>Approved destination</strong>. 7. Compare <strong>Allowed for egress</strong> com <strong>Removed before egress</strong>. 8. Confira <strong>Trusted synthetic values</strong> e <strong>Protected egress payload</strong>. 9. Em ALLOW ou REDACT executável, registre autorização humana quando exigida. 10. Execute pelo Protected Executor. 11. Verifique o estado externo e confirme que Business Outcome só mostra REVOKED — VERIFIED ou DELIVERED — VERIFIED após o read-back específico da ação. 12. Em Evidence, leia summary e outcomes antes da provenance técnica.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Falha de provedor, T3N, Agent Card, A2A, policy KV, Proposal Agent, Protected Executor, <code>checkDelegation()</code> ou trust boundary nunca é apresentada como sucesso. <code>authorised=false</code> resulta em Denied; erro ou resposta inválida resulta em Unknown. <strong>REDACT</strong> só é executável quando o mínimo específico da ação sobrevive e o subconjunto reavaliado vira ALLOW. Payload confiável ausente, chave não solicitada, campo secreto, referência privada obrigatória removida ou valor alterado após autorização falha fechado. <strong>Destination changed</strong> exige nova ação, avaliação e autorização. Aceitação sem read-back permanece pendente ou UNVERIFIED. Para notificação, DELIVERED sem <code>recipient_resolved=true</code> permanece UNVERIFIED. <strong>Audit integrity broken</strong> bloqueia mudanças protegidas. Prompt rejeitado informa somente a categoria detectada, sem ecoar valor ou offset. Sessão expirada exige novo login.</p>
          </section>
        </div>
      </section>
    </div>,
