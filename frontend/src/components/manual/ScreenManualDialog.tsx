      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p><strong>Protection flow</strong> contém cenário, prompt, proposta, decisão, próxima ação permitida, autorização humana, execução, retenção e activity rail. Para <code>notify-security</code>, o painel também mostra a referência lógica privada solicitada, o boundary de resolução, se plaintext fica visível à aplicação e o estado de verificação da entrega. <strong>Evidence</strong> começa com resumo executivo e outcomes observados, deixando hashes, DIDs e provenance técnica em grupos expansíveis. O <strong>Trust Flow</strong> no topo resume AI, Policy, Human, Execute e Verify. <strong>System readiness details</strong> expande o diagnóstico completo sem criar uma segunda fonte de estado. <strong>Refresh status</strong>, <strong>Refresh evidence</strong> e <strong>Refresh audit evidence</strong> são leituras e não repetem remediações.</p>
          </section>
          <section>
          ...
          <section>
            <h3>Policy, autorização humana, valores privados e execução</h3>
            <p><strong>DENY</strong> bloqueia. <strong>REDACT</strong> exige minimização. <strong>ALLOW</strong> permite continuar, mas não equivale a autorização humana. <strong>Policy-allowed destination</strong> significa apenas que um hostname pertence ao conjunto permitido pela policy. <strong>Approved destination</strong> é o hostname exato exibido na proposta e vinculado à autorização humana. A policy pode permitir vários destinos, mas a aprovação humana vale somente para o destino exato mostrado. Antes de registrar autorização, executar ou verificar uma remediação protegida, o backend exige que a cadeia local autenticada continue verificável. A capability vincula action/resource/purpose, campos, referências privadas, policy version/hash, Protected Executor DID e Approved destination.</p>
            <p><strong>Credential revocation</strong> e <strong>security notification</strong> usam a mesma trilha de autorização e o mesmo Protected Executor. A execução lê a URL completa somente do KV privado, exige que o hostname resolvido continue igual ao Approved destination antes de qualquer egress e depois revalida a policy atual. Se o destino mudar, a execução falha fechado e uma nova avaliação e autorização são obrigatórias.</p>
            <p>Em <code>notify-security</code>, a aplicação persiste e transporta somente a referência lógica <code>verified_email</code>. O literal de placeholder T3N é criado somente dentro do contrato Rust/WASM e enviado via <code>http-with-placeholders</code>; frontend, modelo, DTOs do backend e resposta sanitizada não recebem o endereço de e-mail em plaintext. O painel mostra <strong>Resolution boundary: T3N protected execution</strong> e <strong>Plaintext visible to app: NO</strong>. Falha de placeholder, ausência de contexto de usuário ou valor verificado indisponível falham fechado sem ecoar o marker ou o dado privado.</p>
            <p>Aceitação externa não é conclusão. <strong>Verify external state</strong> realiza read-back independente; o host de verificação é provenance técnica separada e não substitui o Approved destination do side effect. Revogação exige <strong>REVOKED</strong>. Notificação exige <strong>DELIVERED</strong> e confirmação booleana de que o recipient privado foi resolvido, sem retornar seu valor. <strong>COMPLETED</strong> só aparece após o estado esperado ser confirmado. Resultado ambíguo, 2xx sem read-back válido ou entrega sem confirmação da resolução privada permanece <strong>UNVERIFIED</strong> e não é reenviado automaticamente.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p>A evidence de autorização registra somente Member state, Effective state e funções/scopes efetivamente consultados para Proposal e Executor. Não persiste grant completo, <code>satisfied</code>, <code>missing</code>, tokens, credenciais, placeholders resolvidos ou valores privados. O live orchestrator falha antes dos cenários se qualquer principal necessário não estiver ACTIVE/Confirmed. O cenário de resolução de <code>verified_email</code> permanece <strong>NOT RUN</strong> até uma execução live explícita com perfil sintético verificado e endpoint controlado; somente depois pode virar PASS. A integridade HMAC local é um controle da aplicação e não é apresentada como attestation T3N ou prova de hardware.</p>
          </section>
          <section>
            <h3>Fluxo principal</h3>
            <p>1. Entre na aplicação. 2. Leia o readiness e o <strong>Current result</strong> no Trust Flow. 3. Abra <strong>System readiness details</strong> quando precisar diagnosticar Tenant, Proposal Agent, Protected Executor, grants ou acesso efetivo. 4. Em <strong>Protection flow</strong>, escolha o cenário e revise o prompt. 5. Use <strong>Analyze with agent</strong>. 6. Inspecione proposta, decisão, referências privadas lógicas e <strong>Approved destination</strong>. 7. Se houver DENY no cenário de credencial comprometida, use <strong>Prepare safe path</strong> para criar somente a proposta mínima. 8. Se a policy estiver PENDING, use o retry dentro da etapa Policy. 9. Em ALLOW, registre autorização humana quando exigida. 10. Confirme novamente o Approved destination e execute pelo Protected Executor. 11. Verifique o estado externo específico da ação: REVOKED para revogação ou DELIVERED + resolução privada confirmada para notificação. 12. Em <strong>Evidence</strong>, leia summary e outcomes antes de abrir provenance técnica. A2A pode reproduzir somente a etapa pública de proposta/avaliação.</p>
          </section>
          <section>
            <h3>Mensagens e estados de erro</h3>
            <p>Falha de provedor, T3N, Agent Card, A2A, policy KV, Proposal Agent, Protected Executor, <code>checkDelegation()</code> ou trust boundary nunca é apresentada como sucesso. <code>authorised=false</code> resulta em Denied; erro ou resposta inválida resulta em Unknown. <strong>Audit integrity broken</strong> bloqueia mudanças protegidas; chave histórica ausente é exibida como KEY_MISMATCH e não como VERIFIED. Troca da Executor DID invalida a capability. Mudança de policy após autorização bloqueia execução. <strong>Destination changed</strong> significa que o hostname configurado no KV privado deixou de corresponder ao Approved destination vinculado à autorização humana; o contrato bloqueia antes do egress e a interface orienta criar/reavaliar e autorizar novamente a ação. Host ausente ou enviado como URL, porta, path ou credencial também falha fechado. Em notificação privada, recipient ausente, placeholder não autorizado, ausência de user context, estado diferente de DELIVERED ou <code>recipient_resolved=false</code> nunca produz COMPLETED. Prompt rejeitado informa somente a categoria detectada, sem ecoar valor ou offset. Sessão expirada exige novo login.</p>
          </section>
        </div>
      </section>
    </div>,
