      ...
      <section ref={dialogRef} tabIndex={-1} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="screen-manual-title" aria-describedby="screen-manual-purpose">
        ...
        <div className="manual-content">
          ...
          <section>
            ...
            <p>A autorização humana é persistida antes da execução. Quando <strong>Execute protected credential revocation</strong> é iniciado, o backend emite um <strong>One-time authorization proof v2</strong> assinado com Ed25519. A chave privada permanece somente no backend; gateway e contrato T3N recebem apenas a chave pública. O proof vincula incident/action/request/decision, action/resource/purpose, <code>normalPayloadHash</code>, campos, referências privadas, policy version/hash, <strong>Approved destination</strong>, Protected Executor DID, emissão, expiração e nonce. O gateway faz uma validação fail-fast, mas o mesmo proof é encaminhado ao <code>execute-remediation</code>, onde o WASM verifica assinatura, validade temporal, bindings e Executor DID e consome o hash do nonce no KV privado T3N antes de ler qualquer segredo de remediação ou permitir HTTP. Proof ausente, expirado, alterado, repetido ou em formato legado falha fechado.</p>
            <p>O indicador <strong>T3N execution proof check</strong> só mostra <strong>VERIFIED</strong> após uma execução ter sido aceita pelo contrato. Ele comprova que o registro assinado de autorização chegou íntegro ao boundary de execução e foi validado pelo WASM; não afirma autenticação biométrica nem identidade humana além do que foi registrado pelo fluxo de autorização. O ledger de replay retém somente hash do nonce e expiração, nunca o proof completo.</p>
            <p>Os valores em <strong>Trusted synthetic values</strong> vêm do backend confiável e são persistidos junto da ação. Alterar qualquer chave ou valor depois da autorização invalida o binding do proof. Campo secreto, chave não solicitada, payload confiável ausente ou mínimo obrigatório removido falham fechado antes do protected HTTP.</p>
            <p><strong>Policy-allowed destination</strong> significa que um hostname pertence ao conjunto permitido pela policy. <strong>Approved destination</strong> é o hostname exato vinculado à autorização humana. A URL completa permanece no KV privado. <strong>Execute protected credential revocation</strong> exige o proof one-time válido, correspondência do hostname, policy atual compatível, <strong>Protected egress payload</strong> formado somente pela interseção entre valores persistidos e <code>allowed_fields</code>, reavaliação do conjunto mínimo e só então chama o endpoint externo. Aceitação externa não é conclusão; <strong>COMPLETED</strong> só aparece após read-back independente do estado esperado.</p>
          </section>
          <section>
