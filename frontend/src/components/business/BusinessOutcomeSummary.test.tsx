...
const INCIDENT: Incident = {
  id: 'incident-1', title: 'Credential compromise', severity: 'CRITICAL', summary: 'Synthetic compromise', source: 'AI_AGENT', status: 'OPEN',
  createdAt: '2026-09-13T15:00:00.000Z', expiresAt: '2026-09-14T15:00:00.000Z', retentionState: 'ACTIVE',
};
...
const THREAT_ACTION: ActionProposal = {
  ...
  fields: ['incident_id', 'credential_id', 'api_key'], normalPayload: {}, privateRefs: [], status: 'EVALUATED', createdAt: '2026-09-13T15:00:00.000Z',
  remediationAuthorizedBy: null, remediationAuthorizedAt: null,
};
...
function policyDecision(action: ActionProposal, value: PolicyDecision['decision'], evaluatedAt = '2026-09-13T15:00:00.420Z'): PolicyDecision {
  ...
  return {
    id: `decision-${action.id}`, actionProposalId: action.id, decision: value, reasonCode: `${value}_TEST`, reason: value,
    ...
    allowedPrivateRefs: value === 'DENY' ? [] : notify ? ['verified_email'] : [], redactedPrivateRefs: [],
    policyVersion: '2026-09-12.1', policyHash: 'a'.repeat(64), requiresHumanAuthorization: value !== 'DENY', evaluatedAt,
  };
}
...
function execution(action: ActionProposal, state: RemediationExecution['state'], completedAt: string | null = null): RemediationExecution {
  return {
    incidentId: INCIDENT.id, actionId: action.id, requestId: action.requestId, state, httpCode: 202,
    operationId: 'operation-1', verificationAttempts: state === 'COMPLETED' ? 2 : 1, failureCode: null,
    startedAt: '2026-09-13T15:00:02.000Z', completedAt,
  };
}

function renderSummary(overrides: Partial<Parameters<typeof BusinessOutcomeSummary>[0]> = {}) {
  render(<BusinessOutcomeSummary scenario={DEFAULT_ENTERPRISE_SCENARIO} incident={null} selectedAction={null} decision={null} remediationExecution={null} agentAnalysis={null} {...overrides} />);
}
...
describe('BusinessOutcomeSummary', () => {
  ...
  it('shows executable REDACT as minimized and still requiring human authorization', () => {
    const currentAction = { ...SAFE_ACTION, status: 'EVALUATED' as const, fields: [...SAFE_ACTION.fields, 'employee_department'], normalPayload: { ...SAFE_ACTION.normalPayload, employee_department: 'finance' } };
    const currentDecision = policyDecision(currentAction, 'REDACT');
    renderSummary({ incident: INCIDENT, selectedAction: currentAction, decision: currentDecision });
  ...
  it('keeps accepted execution pending until independent verification completes', () => {
    const authorized = { ...SAFE_ACTION, status: 'REMEDIATION_AUTHORIZED' as const, remediationAuthorizedBy: 'ops-reviewer', remediationAuthorizedAt: '2026-09-13T15:00:01.500Z' };
  ...
  it('shows verified revocation state and measured duration only after COMPLETED', () => {
    const remediated = { ...SAFE_ACTION, status: 'REMEDIATED' as const, remediationAuthorizedBy: 'ops-reviewer', remediationAuthorizedAt: '2026-09-13T15:00:01.500Z' };
  ...
  it('shows DELIVERED VERIFIED only after completed notification read-back', () => {
    const remediated = { ...NOTIFY_ACTION, status: 'REMEDIATED' as const, remediationAuthorizedBy: 'ops-reviewer', remediationAuthorizedAt: '2026-09-13T15:00:01.500Z' };
