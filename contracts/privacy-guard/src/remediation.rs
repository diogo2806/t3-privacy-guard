...
pub fn execute_remediation(input: &[u8]) -> Result<Vec<u8>, String> {
    let request: RemediationExecutionRequest = serde_json::from_slice(input).map_err(|_| "execute-remediation: invalid JSON input".to_string())?;
    #[cfg(target_arch = "wasm32")]
    {
        let result = execute_wasm(request)?;
        return serde_json::to_vec(&result).map_err(|_| "execute-remediation: failed to encode result".to_string());
    }
...
#[cfg(target_arch = "wasm32")]
use crate::host::{interfaces::{http_with_placeholders as hwp, kv_store, logging}, tenant::tenant_context};
...
fn execute_wasm(request: RemediationExecutionRequest) -> Result<RemediationResult, String> {
    ...
    let response = hwp::call(&hwp::Request {
        ...
        url: api_url,
        headers: Some(vec![("Authorization".to_string(), alloc::format!("Bearer {api_key}")), ("Accept".to_string(), "application/json".to_string())]),
    ...
    let operation_id = serde_json::from_slice::<serde_json::Value>(&response.payload).ok()
        .and_then(|value| value.get("operation_id").and_then(|v| v.as_str()).map(str::to_string));
    Ok(RemediationResult { request_id: policy_request.request_id, status: "COMPLETED".to_string(), http_code: response.code, operation_id })
}
...
fn read_secret(key: &str) -> Result<String, String> {
    ...
    let bytes = kv_store::get(&map_name, key.as_bytes()).map_err(|_| "private secret map is unavailable".to_string())?.ok_or_else(|| alloc::format!("required private configuration {key} is missing"))?;
...
fn extract_https_host(url: &str) -> Result<&str, String> {
    let rest = url.strip_prefix("https://").ok_or("security_api_url must use https")?;
    let host = rest.split('/').next().unwrap_or("");
    if host.is_empty() || host.contains('@') || host.contains(':') { return Err("security_api_url contains an invalid host".to_string()); }
    Ok(host)
}
...
fn format_http_error(error: hwp::HttpError) -> String {
    match error {
        ...
        hwp::HttpError::UpstreamError(_) => "remediation upstream transport failed".to_string(),
    }
}
...
mod tests {
    use super::*;
    #[test] fn native_execution_never_simulates_secret_egress() {
        let input = serde_json::to_vec(&RemediationExecutionRequest { request_id:"r1".into(), agent_did:"did:t3n:a".into(), action:"revoke-credential".into(), resource:"credential:test".into(), purpose:"incident-remediation".into(), fields:vec!["incident_id".into(),"credential_id".into(),"reason".into()] }).unwrap();
