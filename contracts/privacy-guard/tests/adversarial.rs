...
fn invalid_request_id_is_denied() {
    ...
    assert_eq!(evaluate(&blank).reason_code, "INVALID_REQUEST_ID");

    let mut oversized = base_request();
    oversized.request_id = "x".repeat(129);
    assert_eq!(evaluate(&oversized).reason_code, "INVALID_REQUEST_ID");
}
...
fn legitimate_remediation_remains_allowed_after_blocked_attack() {
    ...
    assert_eq!(evaluate(&attack).decision, Decision::Deny);
